...
public:
    ...
    {
        ostringstream oss;
        oss << proc_ptr->processor_info() << "Attempt to create duplicate store object with name: " << obj_name;
        msg = oss.str();
    }
};
...
public:
    ...
    {
        ostringstream oss;
        oss << proc_ptr->processor_info() << "Attempt to access unknown store object with id: " << obj_name;
        msg = oss.str();
    }
};
...
void Visual_store::make_object_appear(const Symbol& name, GU::Point location, GU::Size size)
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "make object " << ' ' << name << " appear at " << location << " size " << size
                  << endl;
...
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "make object " << obj_name << " disappear" << endl;
...
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "make object " << obj_name << " reappear" << endl;
...
GU::Point Visual_store::set_object_location(const Symbol& obj_name, GU::Point location)
{
    ...
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set object " << obj_name << " location to " << location << endl;
...
GU::Size Visual_store::set_object_size(const Symbol& obj_name, GU::Size size)
{
    ...
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set object " << obj_name << " size to " << size << endl;
    shared_ptr<Visual_store_object> obj_ptr = get_object_ptr(obj_name); // throws if not there
...
Symbol Visual_store::set_object_property(const Symbol& obj_name, const Symbol& prop_name, const Symbol& prop_value)
{
    changed = false;

    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set object " << obj_name << " property " << prop_name << " to " << prop_value
                  << endl;
...
Symbol_list_t Visual_store::find_all(const Symbol_list_t& pv_list) const
{
    ...
    for (Store_container_t::const_iterator obj_iter = objects.begin(); obj_iter != objects.end(); obj_iter++) {
