...
void Stream_tracker_base::create_initial_streams()
{
    ...
    // set stream assignment for each word, and update each stream with the assigned word - we don't need identity of
    // current word for this process
    int stream_index = 0;
    for (auto it = sounds.begin(); it != sounds.end(); ++it, stream_index++) {
        //        std::shared_ptr<Stream_base> new_stream =
        //        Two_point_linear_stream::create(concatenate_to_Symbol("Stream", stream_index+1));
        //        std::shared_ptr<Stream_base> new_stream = Averaging_stream::create(concatenate_to_Symbol("Stream",
        //        stream_index+1));
        std::shared_ptr<Stream_base> new_stream =
            create_stream(stream_type_name, (concatenate_to_Symbol("Stream", stream_index + 1)));
...
{
    auto it = find_if(streams.begin(), streams.end(), [stream_name](std::shared_ptr<Stream_base> the_stream) {
        return the_stream->get_name() == stream_name;
    });
