...
// Reset - Put everything back to its initial state and remove any Temporal WM items.
{
    ...
    if (numTicks >= 0) // If numTicks < 0 then there is no WM item to remove.
    {
        ...
        Coordinator::get_instance().schedule_event(
            new Cognitive_Delete_Clause_event(get_time(), get_human_ptr()->get_cognitive_ptr(), removal_description));
    }
    ...
    if (get_time() == honorIncrementAtTime) {
        ...
        if (numTicks >= 0) // If numTicks < 0 then there is no WM item to remove.
        {
            ...
            Coordinator::get_instance().schedule_event(new Cognitive_Delete_Clause_event(
                get_time(), get_human_ptr()->get_cognitive_ptr(), removal_description));
        }
        ...
        Coordinator::get_instance().schedule_event(
            new Cognitive_Add_Clause_event(get_time(), get_human_ptr()->get_cognitive_ptr(), add_description));
