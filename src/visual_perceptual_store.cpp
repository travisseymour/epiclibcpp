...
If an object disappears:
    ...
    schedule a first Erase event to arrive after unsupported_object_decay_time, and set the disappearance time to this
time also If an object erase event arrives: If the object is not present - ignore with a trace message If the
disappearance time is 0 (cancelled) or > current_time, ignore this erase event - do nothing (no message) Do one of the
following depending on the state of the object: The object is UNSUPPORTED: set the Status to Disappearing and inform
cogp and the views set the state to RETAINED schedule a second Erase event to arrive after retained_object_decay_time,
and set the disappearance time to this time do nothing more The object is REATAINED: remove it from the store and inform
cogp and the views and remove it from the name map! The object is PRESENT: ignore with a trace message
...
void Visual_perceptual_store::handle_event(const Visual_Appear_event* event_ptr)
{
    ...
    // if it is already present, it is being reinstated, so we need to change its state to present and Status Property
    // to Visible
    else {
        if (get_trace() && Trace_out)
...
void Visual_perceptual_store::handle_event(const Visual_Erase_event* event_ptr)
{
    ...
    if (!is_present(event_ptr->object_name)) {
        if (get_trace() && Trace_out)
            Trace_out << processor_info() << event_ptr->object_name << " no longer present for erasure" << endl;
        return;
    }
    ...
    if (disappearing_time == 0 || disappearing_time > get_time()) {
        if (get_trace() && Trace_out)
            Trace_out << processor_info() << event_ptr->object_name << " erase event discarded or superseded" << endl;
        return;
    }
    ...
    case Visual_perceptual_object::UNSUPPORTED: {
        if (get_trace() && Trace_out)
            Trace_out << processor_info() << event_ptr->object_name << " disappearing but being retained in memory"
                      << endl;
        ...
        if(!update_retained_objects()) {
            if(get_trace() && Trace_out)
                Trace_out << processor_info() << event_ptr->object_name  << " and other objects lost from memory " <<
        endl;
            }
        else {
...
void Visual_perceptual_store::lose_object_from_memory(shared_ptr<Visual_perceptual_object> obj_ptr)
{
    ...
    case Visual_perceptual_object::PRESENT:
        ...
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Visual_c, object_name, Status_c,
                                                                               Reappearing_c, Nil_c);
    case Visual_perceptual_object::UNSUPPORTED: {
        // just update the cognitive processor, because the object is going to be removed
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Visual_c, object_name, Status_c,
    ...
    Symbol_list_t ::iterator it_value = prop_values.begin();
    for (; it_name != prop_names.end() && it_value != prop_values.end(); ++it_name, ++it_value) {
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Visual_c, object_name, *it_name,
                                                                               *it_value, Nil_c);
    }
    ...
    if (!(name_map.is_physical_name_in_use(physical_name))) {
        if (get_trace() && Trace_out)
            Trace_out << processor_info() << "Object names " << physical_name << '/' << object_name
                      << " removed from Visual system" << endl;
        name_map.remove_names_with_psychological_name(object_name);
    }
    else {
        if (get_trace() && Trace_out)
            Trace_out << processor_info() << "Object names " << physical_name << '/' << object_name
                      << " being retained in Visual system" << endl;
    }
...
void Visual_perceptual_store::handle_event(const Visual_Change_Location_event* event_ptr)
{
    ...
    if (changed) {
        ...
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Visual_c, event_ptr->object_name,
                                                                               Location_c, Nil_c, Change_c);

        // schedule for later delivery
        schedule_event(new Cognitive_Update_event(get_time() + change_decay_time.get_long_value(),
                                                  get_human_ptr()->get_Cognitive_processor_ptr(), Visual_c,
                                                  event_ptr->object_name, Location_c, Change_c, Nil_c));
    }
}
...
void Visual_perceptual_store::handle_event(const Visual_Change_Size_event* event_ptr)
{
    ...
    if (changed) {
        // immediately post the change
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Visual_c, event_ptr->object_name, Size_c,
                                                                               Nil_c, Change_c);

        // schedule for later delivery
        schedule_event(new Cognitive_Update_event(get_time() + change_decay_time.get_long_value(),
                                                  get_human_ptr()->get_Cognitive_processor_ptr(), Visual_c,
                                                  event_ptr->object_name, Size_c, Change_c, Nil_c));
    }
}
...
// KLUDGE KLUDGE KLUDGE
    ...
    if ((event_ptr->property_name == Color_c || event_ptr->property_name == Shape_c) && new_value == Nil_c) {
        ...
        if (new_value == Nil_c && old_value != Nil_c && event_ptr->property_name != Detection_c) {
            ...
            schedule_event(new Visual_Erase_Property_event(disappearing_time, this, event_ptr->object_name,
...
void Visual_perceptual_store::update_property(const Symbol& object_name, const Symbol& property_name,
                                              const Symbol& new_value)
{
    ...
    // send through to cognitive processor immediately
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Visual_c, object_name, property_name,
                                                                           old_value, new_value);
}
...
void Visual_perceptual_store::handle_event(const Visual_Erase_Property_event* event_ptr)
{
    ...
    if (!is_present(event_ptr->object_name)) {
        if (get_trace() && Trace_out)
            Trace_out << processor_info() << event_ptr->object_name << " no longer present for erasure of "
                      << event_ptr->property_name << endl;
        return;
    }
    ...
    // if so, do the erasure and removal from the cognitive processor store
        ...
        if (get_trace() && Trace_out)
            Trace_out << processor_info() << event_ptr->object_name << " losing property " << event_ptr->property_name
                      << endl;
        ...
        /*		// update the store and broadcast the change
                Symbol old_value = Visual_store::set_object_property(event_ptr->object_name, event_ptr->property_name,
           Nil_c);
                // send through to cognitive processor immediately
                get_human_ptr()->get_Cognitive_processor_ptr()->
                    update_object_property(Visual_c, event_ptr->object_name, event_ptr->property_name,
        ...
        */
        ...
        throw Epic_internal_error(this, string("Unexpected Visual_Erase_Property_event for ") +
                                            event_ptr->object_name.str() + " property " +
                                            event_ptr->property_name.str());
}