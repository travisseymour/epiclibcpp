...
Combining_Node* Rule_Node::create(Production_System* production_system_ptr, const Symbol& rule_name,
                                  Combining_Node* source, const Variable_list& var_names,
                                  const Predicate_ptr_list_t& predicates, const list_Symbol_list_t& add_action_list,
                                  const list_Symbol_list_t& delete_action_list,
                                  const list_Symbol_list_t& command_action_list,
                                  const list_Symbol_list_t& add_command_action_list,
                                  const list_Symbol_list_t& add_command_pattern_list)
{
    ...
    if (var_names.size() == 0)
        return new Simple_Rule_Node(production_system_ptr, Combining_Node::get_next_node_number(), rule_name, source,
                                    add_action_list, delete_action_list, command_action_list, add_command_action_list,
                                    add_command_pattern_list);
    else if (var_names.size() > 0 && predicates.size() == 0)
        return new Bindings_Rule_Node(production_system_ptr, Combining_Node::get_next_node_number(), rule_name, source,
                                      add_action_list, delete_action_list, command_action_list, add_command_action_list,
                                      add_command_pattern_list);
    //	(var_names.size() > 0 && predicates.size() > 0)
    return new Bindings_Predicate_Rule_Node(production_system_ptr, Combining_Node::get_next_node_number(), rule_name,
                                            source, predicates, add_action_list, delete_action_list,
                                            command_action_list, add_command_action_list, add_command_pattern_list);
}
    ...
    for (Symbol_list_t::const_iterator it = pattern.begin(); it != pattern.end(); it++) {
        if (is_variable(*it)) {
            ...
            if (var_value == no_value) {
                string msg = string("In rule ") + rule_name.str() + string(": variable ") + (*it).str() +
                             string(" has no value");
...
void Simple_Rule_Node::execute(Command_interface& command_interface) const
{
    ...
    for (list_Symbol_list_t::const_iterator it1 = add_action_patterns.begin(); it1 != add_action_patterns.end();
         it1++) {
        if (output)
    ...
    for (list_Symbol_list_t::const_iterator it2 = delete_action_patterns.begin(); it2 != delete_action_patterns.end();
         it2++) {
        if (output)
...
void Bindings_Rule_Node::execute_with_bindings(const Binding_set_list_t& binding_set_list,
                                               Command_interface& command_interface) const
{
    bool output = production_system_ptr->get_output_run_messages() && PPS_out;
    if (output)
        PPS_out << "*** Fire: " << rule_name << endl;

    // for each binding set, and for each pattern in add patterns, add the add clauses, ditto with delete patterns and
    // commands
    for (Binding_set_list_t::const_iterator bs_it = binding_set_list.begin(); bs_it != binding_set_list.end();
         bs_it++) {
        if (output)
        ...
        for (list_Symbol_list_t::const_iterator it2 = delete_action_patterns.begin();
             it2 != delete_action_patterns.end(); it2++) {
            Symbol_list_t result;
        ...
        /*
         for(list_Symbol_list_t::const_iterator it = add_command_actions.begin(); it != add_command_actions.end(); it++)
         { Symbol_list_t result; instantiate_pattern(add_command_patterns.front(), *bs_it, result);
