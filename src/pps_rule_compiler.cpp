...
{
    for (Compiler_rule_data_list_t::iterator rule_it = rule_data_list.begin(); rule_it != rule_data_list.end();
         rule_it++) {
        ...
        for (list_list_Symbol_list_t::iterator neg_it = neg_pattern_list.begin(); neg_it != neg_pattern_list.end();
             neg_it++) {
        ...
        //		PPS_out << neg_pattern_list << endl;
        for (list_list_Symbol_list_t::iterator neg_it = neg_conj_pattern_list.begin();
             neg_it != neg_conj_pattern_list.end(); neg_it++) {
            Compiler_node_data_list_t neg_node_data_list;
...
void build_discrimination_net_from_patterns(Production_System* production_system_ptr, list_Symbol_list_t& patterns,
                                            Compiler_node_data_list_t& node_data_list)
{
...
void combine_rules(Production_System* production_system_ptr, Compiler_rule_data_list_t& rule_data_list)
{
    ...
    for (Compiler_rule_data_list_t::iterator rule_it = rule_data_list.begin(); rule_it != rule_data_list.end();
        ...
        Combining_Node* new_node_ptr = Rule_Node::create(
            production_system_ptr, (*rule_it).rule_name, node_datum.node_ptr, node_datum.varlist, predicate_list,
            (*rule_it).add_pattern_list, (*rule_it).delete_pattern_list, (*rule_it).command_action_list,
            (*rule_it).add_command_action_list, (*rule_it).add_command_pattern_list);
...
Predicate_ptr_list_t build_predicate_ptr_list(const list_Symbol_list_t& predicate_patterns)
{
    ...
    for (list_Symbol_list_t::const_iterator pred_it = predicate_patterns.begin(); pred_it != predicate_patterns.end();
         pred_it++) {
...
void replace_pair(Compiler_rule_data_list_t& rule_data_list, const Compiler_node_data_pair& max_pair,
                  const Compiler_node_data& replacement)
{
    ...
    for (Compiler_rule_data_list_t::iterator rule_it = rule_data_list.begin(); rule_it != rule_data_list.end();
        ...
        for (list_Compiler_node_data_list_t::iterator neg_it = (*rule_it).neg_node_data_lists.begin();
             neg_it != (*rule_it).neg_node_data_lists.end(); neg_it++) {
            replace_pair_in_list((*neg_it), max_pair, replacement);
        }
        for (list_Compiler_node_data_list_t::iterator neg_it = (*rule_it).neg_conjunction_node_data_lists.begin();
...
void replace_pair_in_list(Compiler_node_data_list_t& node_data_list, const Compiler_node_data_pair& org_pair,
                          const Compiler_node_data& replacement)
{
    Compiler_node_data_list_t::const_iterator found_it1 =
        find(node_data_list.begin(), node_data_list.end(), org_pair.first);
    // might be a faster way to do this
    if (found_it1 == node_data_list.end())
        return;
    Compiler_node_data_list_t::const_iterator found_it2 =
        find(node_data_list.begin(), node_data_list.end(), org_pair.second);
...
// replacing a pos_neg_pair means that the new negation node replaces the positive item, while the negated node is
// removed.

void replace_pos_neg_pair(Compiler_rule_data_list_t& rule_data_list, const Compiler_node_data_pair& org_pair,
                          const Compiler_node_data& replacement)
{
    // remove the max_pair terms from the patterns and replace with a dummy pattern
    for (Compiler_rule_data_list_t::iterator rule_it = rule_data_list.begin(); rule_it != rule_data_list.end();
        ...
        Compiler_node_data_list_t::const_iterator found_it1 =
            find((*rule_it).pos_node_data_list.begin(), (*rule_it).pos_node_data_list.end(), org_pair.first);
        // might be a faster way to do this
        if (found_it1 == (*rule_it).pos_node_data_list.end())
            continue;
        Compiler_node_data_list_t::const_iterator found_it2 =
            find((*rule_it).neg_node_data_list.begin(), (*rule_it).neg_node_data_list.end(), org_pair.second);
...
void print_rule_data_list(const Compiler_rule_data_list_t& rule_data_list)
{
    ...
    for (Compiler_rule_data_list_t::const_iterator rule_it = rule_data_list.begin(); rule_it != rule_data_list.end();
         rule_it++) {
...
void display_network_nodes(const Compiler_rule_data_list_t& rule_data_list)
{
    ...
    for (Compiler_rule_data_list_t::const_iterator rule_it = rule_data_list.begin(); rule_it != rule_data_list.end();
        ...
        for (Compiler_node_data_list_t::const_iterator it2 = (*rule_it).pos_node_data_list.begin();
             it2 != (*rule_it).pos_node_data_list.end(); it2++) {
