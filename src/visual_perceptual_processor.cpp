...
void Visual_perceptual_processor::make_object_appear(const Symbol& name, GU::Point location, GU::Size size)
{
    ...
    schedule_event(
        new Visual_Appear_event(appear_time, get_human_ptr()->get_visual_perceptual_store_ptr(), name, location, size));
    // create and send a transient property forward
    schedule_event(new Visual_Change_Property_event(appear_time, get_human_ptr()->get_visual_perceptual_store_ptr(),
                                                    name, Detection_c, Onset_c));
    // arrange to remove the transient after its delay
    schedule_event(new Visual_Change_Property_event(appear_time + onset_offset_decay_time.get_long_value(),
                                                    get_human_ptr()->get_visual_perceptual_store_ptr(), name,
                                                    Detection_c, Nil_c));

    // send the "Visible" property forward after its own delay
    long visible_time = get_time() + visible_delay.get_long_value();
    schedule_event(new Visual_Change_Property_event(visible_time, get_human_ptr()->get_visual_perceptual_store_ptr(),
...
void Visual_perceptual_processor::make_object_disappear(const Symbol& name)
{
    ...
    schedule_event(
        new Visual_Disappear_event(disappear_time, get_human_ptr()->get_visual_perceptual_store_ptr(), name));
    // create and send a transient property forward
    schedule_event(new Visual_Change_Property_event(disappear_time, get_human_ptr()->get_visual_perceptual_store_ptr(),
                                                    name, Detection_c, Offset_c));
    // arrange to remove the transient after its delay
    schedule_event(new Visual_Change_Property_event(disappear_time + onset_offset_decay_time.get_long_value(),
                                                    get_human_ptr()->get_visual_perceptual_store_ptr(), name,
                                                    Detection_c, Nil_c));
}
...
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set object " << name << " location to " << location << endl;
...
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set object " << name << " size to " << size << endl;

    // transmit the event forward to the next store
    schedule_event(new Visual_Change_Size_event(get_time() + size_delay.get_long_value(),
                                                get_human_ptr()->get_visual_perceptual_store_ptr(), name, size));
}
/*
...
void Visual_perceptual_processor::set_object_property(const Symbol& object_name, const Symbol& property_name,
                                                      const Symbol& property_value)
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set object " << object_name << " property " << property_name << " to "
                  << property_value << endl;
    /*
        // special case for Detection Onset and Offset properties
        if(property_name == Detection_c && (property_value == Onset_stationary_c || property_value ==
       Offset_stationary_c)) {
            // create and send a transient property forward
    ...
    */
    ...
    if (visual_encoder_ptr &&
        visual_encoder_ptr->set_object_property(object_name, property_name, property_value, recoding_time))
        return;

    // transmit the event forward to the next store
    schedule_event(new Visual_Change_Property_event(get_time() + recoding_time,
                                                    get_human_ptr()->get_visual_perceptual_store_ptr(), object_name,
...
void Visual_perceptual_processor::handle_event(const Visual_Delay_event* event_ptr)
{
    ...
    if (object_time != event_ptr->get_time() || object_property_value != property_value) {
        if (get_trace() && Trace_out)
            Trace_out << processor_info() << "Delay event discarded for " << object_name
                      << " because event time and value " << event_ptr->get_time() << " " << property_value
                      << " does not match stored " << object_time << " " << object_property_value << endl;
        return;
    }
    ...
    schedule_event(new Visual_Change_Property_event(get_time(), get_human_ptr()->get_visual_perceptual_store_ptr(),
