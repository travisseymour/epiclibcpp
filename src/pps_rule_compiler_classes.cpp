...
ostream& operator<<(ostream& os, const Compiler_rule_data& datum)
{
    ...
    if (datum.pos_node_data_list.size() > 0) {
        ...
        for (Compiler_node_data_list_t::const_iterator it2 = datum.pos_node_data_list.begin();
             it2 != datum.pos_node_data_list.end(); it2++) {
            os << (*it2) << endl;
        }
    }
    ...
    if (datum.neg_node_data_lists.size() > 0) {
        ...
        for (list_Compiler_node_data_list_t::const_iterator neg_it = datum.neg_node_data_lists.begin();
             neg_it != datum.neg_node_data_lists.end(); neg_it++) {
            os << "(";
    ...
    if (datum.neg_conjunction_node_data_lists.size() > 0) {
        ...
        for (list_Compiler_node_data_list_t::const_iterator neg_it = datum.neg_conjunction_node_data_lists.begin();
             neg_it != datum.neg_conjunction_node_data_lists.end(); neg_it++) {
            os << "(";
            for (Compiler_node_data_list_t::const_iterator it3 = (*neg_it).begin(); it3 != (*neg_it).end(); it3++) {
                os << (*it3) << ' ';
            }
    ...
    if (datum.neg_node_data_list.size() > 0) {
        ...
        for (Compiler_node_data_list_t::const_iterator it4 = datum.neg_node_data_list.begin();
             it4 != datum.neg_node_data_list.end(); it4++) {
            os << (*it4) << endl;
        }
    }
    ...
    if (datum.delete_pattern_list.size() > 0) {
        ...
        for (list_Symbol_list_t::const_iterator it6 = datum.delete_pattern_list.begin();
             it6 != datum.delete_pattern_list.end(); it6++) {
            os << (*it6) << endl;
        }
    }
    if (datum.command_action_list.size() > 0) {
        os << "Motor actions:" << endl;
        for (list_Symbol_list_t::const_iterator it7 = datum.command_action_list.begin();
             it7 != datum.command_action_list.end(); it7++) {
            os << (*it7) << endl;
        }
    }
    return os;
