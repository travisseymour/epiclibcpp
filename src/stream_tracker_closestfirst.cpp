...
void Stream_tracker_ClosestFirst::assign_sounds_to_streams()
{
    ...
    for (int sound_index = 0; sound_index < sounds.size(); sound_index++)
        for (int stream_index = 0; stream_index < sounds.size(); stream_index++) {
            double distance = get_distance(sounds[sound_index]->get_pitch(), sounds[sound_index]->get_loudness(),
    ...
    for (int assignments = 0; assignments < sounds.size(); assignments++) {
        double closest_distance = DBL_MAX; // std::numeric_limits<double>::max(); // huge ugly kludge value
                                           // numeric_limits<double>::max()  <limits>
...
double Stream_tracker_ClosestFirst::get_distance(double pitch, double loudness,
