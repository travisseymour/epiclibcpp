...
// #include "GOMS_../Architecture Classes/Cognitive_processor.h"
...
Human_processor::Human_processor(const std::string& name_)
    ...
    , visual_physical_store_ptr(0)
    , eye_ptr(0)
    , visual_sensory_store_ptr(0)
    , visual_perceptual_processor_ptr(0)
    , visual_perceptual_store_ptr(0)
    , auditory_physical_store_ptr(0)
    , ear_ptr(0)
    , auditory_sensory_store_ptr(0)
    , auditory_perceptual_processor_ptr(0)
    , auditory_perceptual_store_ptr(0)
    , cognitive_ptr(0)
    , manual_ptr(0)
    , ocular_ptr(0)
    , invol_ocular_ptr(0)
    ...
    , trace_all(false)
    , trace_visual(false)
    , trace_auditory(false)
    , trace_cognitive(false)
    , trace_manual(false)
    , trace_ocular(false)
    , trace_vocal(false)
    , trace_temporal(false)
{
...
void Human_processor::set_parameter(const std::string& proc_name, const std::string& param_name,
...
Processor* Human_processor::get_visual_physical_store_ptr() const
{
    return visual_physical_store_ptr;
}

Processor* Human_processor::get_eye_ptr() const
{
    return eye_ptr;
}

Processor* Human_processor::get_visual_sensory_store_ptr() const
{
    return visual_sensory_store_ptr;
}

Processor* Human_processor::get_visual_perceptual_processor_ptr() const
{
    return visual_perceptual_processor_ptr;
}

Processor* Human_processor::get_visual_perceptual_store_ptr() const
{
    return visual_perceptual_store_ptr;
}

Processor* Human_processor::get_auditory_physical_store_ptr() const
{
    return auditory_physical_store_ptr;
}

Processor* Human_processor::get_ear_ptr() const
{
    return ear_ptr;
}

Processor* Human_processor::get_auditory_sensory_store_ptr() const
{
    return auditory_sensory_store_ptr;
}

Processor* Human_processor::get_auditory_perceptual_processor_ptr() const
{
    return auditory_perceptual_processor_ptr;
}

Processor* Human_processor::get_auditory_perceptual_store_ptr() const
{
...
Processor* Human_processor::get_ocular_ptr() const
{
    return ocular_ptr;
}

Processor* Human_processor::get_invol_ocular_ptr() const
{
...
Processor* Human_processor::get_temporal_ptr() const
{
    return temporal_ptr;
}
...
void Human_processor::set_visual_object_property(const Symbol& obj_name, const Symbol& propname,
                                                 const Symbol& propvalue)
{
...
{
    auditory_physical_store_ptr->make_sound_event(name, stream_name, location, timbre, loudness, duration,
                                                  intrinsic_duration);
}
/*
...
{
    auditory_physical_store_ptr->make_speech_event(sound_name, stream_name, content, speaker_gender, speaker_id,
loudness, duration);
}
*/
...
void Human_processor::initialize_recorders()
{
    ...
    for_each(recorder_list.begin(), recorder_list.end(), mem_fn(&Processor::initialize));
