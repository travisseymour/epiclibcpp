...
shared_ptr<Motor_action> Vocal_action::create(Human_processor* human_ptr, Symbol_list_t arguments,
...
{
    human_ptr->schedule_event(
        new Motor_Command_Action_event(time, human_ptr->get_Vocal_processor_ptr(), shared_from_this()
...
Vocal_Speak_action::Vocal_Speak_action(Human_processor* hum_ptr, Symbol_list_t arguments, bool overt_,
                                       bool execute_when_prepared)
    : Vocal_action(hum_ptr, execute_when_prepared)
    , overt(overt_)
{
...
long Vocal_Speak_action::prepare(long base_time)
{
    ...
    shared_ptr<Vocal_Speak_action> ptr =
        std::dynamic_pointer_cast<Vocal_Speak_action>(get_processor_ptr()->get_previous_ptr());
...
long Vocal_Speak_action::execute(long base_time)
{
    ...
    if (overt) {
        Coordinator::get_instance().schedule_event(
            new Device_Vocal_event(base_time, get_device_ptr(), utterance, duration));
    }
    ...
    //	auditory_physical_store_ptr->make_speech_sound_event(Symbol("Internal_speech_item"), Symbol("Overt"), utterance,
    // loudness, duration);
    ...
    */
    ...
    Coordinator::get_instance().schedule_event(new Auditory_Speech_event(base_time, auditory_physical_store_ptr, word));
    ...
    Coordinator::get_instance().schedule_event(
        new Cognitive_Add_Clause_event(movement_completion_time, cognitive_ptr, movement_description));
    Coordinator::get_instance().schedule_event(
        new Cognitive_Delete_Clause_event(movement_completion_time + get_processor_ptr()->get_efferent_deletion_delay(),
