// refactoring 4/30/03 - Model should know only about Human_processor and Device_processor - shouldn't need anything
// else!
...
Model::Model(Device_base* device_ptr_)
    ...
    , device_processor_ptr(0)
    , human_ptr(0)
    , prs_filename_set(false)
    , compiled(false)
    , prs_filename_unchanged(false)
    , preserve_break_names(true)
    , initialized(false)
    , running(false)
...

{
    ...
    try {
        // create the Human_processor and Device_base class device
...
// Travis Seymour March 2023 -- make model do it's own linkage with device
// to avoid having to do this across Python and C++ in pybind11
void Model::interconnect_device_and_human()
{
    device_processor_ptr = device_ptr->get_device_proc_ptr();
    device_processor_ptr->connect(get_human_ptr());
    get_human_ptr()->connect(device_processor_ptr);
    Trace_out << "Interconnecting Device and Human Model" << endl;
    Trace_out << "--------------------------------------" << endl;
    Trace_out << "device_ptr " << "(" << typeid(device_ptr).name() << ")" << endl;
    Trace_out << "device_processor_ptr " << device_processor_ptr->processor_info() << " ("
              << typeid(device_processor_ptr).name() << ")" << endl;
}

// Travis Seymour March 2023 -- made model return the production system
// so I don't have to do that from Python
// Parsimonious_Production_System::Production_System Model::get_production_system_ptr() {
//     return get_human_ptr()->get_Cognitive_processor_ptr()->get_production_system_ptr();
// }
