...
When it and the processors are initialized, it goes into the INITIALIZED state.
...
public:
    ...
    void pause();
    // stop the simulation unconditionally by setting the running to false; deliveries will stop, and processor shut
    // down
    void stop();
    // Atomic-safe accessor for state
    Simulation_state_e get_state() const noexcept
    {
