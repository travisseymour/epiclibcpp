...
public:
    Eye_processor(Human_processor* human_ptr_)
        ...
        eccentricity_delay("Eccentricity_transduction_time", 50)
        , // longer than appearance_delay
        ...
        , inform_invol_ocular_delay("Inform_invol_ocular_delay", 20)
        , psychobj_counter(0) //,
    ...
    {
        auditory_reflex_enabled = r;
        auditory_reflex_on = r;
    }
...
private:
    ...
    bool apply_availability(std::shared_ptr<Visual_store_object> obj_ptr, const Symbol prop_name, Symbol& prop_value,
                            long& delay);
    void send_eccentricity_update(const Symbol& psychological_name, double ecc);
    void send_property_availability_update(std::shared_ptr<Visual_store_object> obj_ptr, const Symbol& prop_name,
