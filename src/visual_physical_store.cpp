...
void Visual_physical_object::update_eccentricity_and_visibility(GU::Point eye_location, double periphery_radius)
{
    ...
    if (eccentricity < periphery_radius || closest_distance(eye_location, location, size) < periphery_radius)
        visible = true;
    else
        visible = false;
}
...
void Visual_physical_store::make_object_appear(const Symbol& physical_name, GU::Point location, GU::Size size)
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "make object " << physical_name << " appear at " << location << " size "
                  << size << endl;
...
Symbol Visual_physical_store::set_object_property(const Symbol& physical_name, const Symbol& propname,
