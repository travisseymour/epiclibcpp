...
public:
    ...
    // present() first presents all the objects, and then presents their properties as set by Object_widget's
    // set_property. present_object() can certainly present object properties, but should not present relational
    // properties unless it can guarantee that the objects appear before the relation properties are set. This is to
    // ensure that any relational properties, such as Above, that refer to another object by name, are added after the
    // relevant objects are known.
    ...
    {
        screen_distance_units = distance_;
        compute_visual_constants();
    }
    static void set_screen_pixels_per_unit(double pixels_per_unit_)
    {
        screen_pixels_per_unit = pixels_per_unit_;
        compute_visual_constants();
    }
    ...
    {
        screen_pixel_visual_origin = screen_pixel_visual_origin_;
        compute_visual_constants();
    }
...
// A Polygon Widget sets the shape to correspond to the supplied polygon - at present, should not be moved or resized,
// or reshaped
class Polygon_widget : public Object_widget
{
public:
    Polygon_widget(Device_base* device_ptr_, const Symbol& widget_name_, const GU::Polygon& polygon_,
                   const Symbol& color_ = Black_c);
};

// A Line Widget sets the shape to correspond to the supplied Line_segment - at present, should not be moved or resized,
// or reshaped
class Line_widget : public Object_widget
{
public:
    Line_widget(Device_base* device_ptr_, const Symbol& widget_name_, const GU::Line_segment& line,
                const Symbol& color_ = Black_c);
};
...
public:
    ...
    Label_widget(Device_base* device_ptr_, const Symbol& widget_name_, GU::Point location_, const Symbol& label_,
                 const Symbol& color_ = Black_c)
        : Widget(device_ptr_, widget_name_, location_, Widget::string_to_size(label_))
        , label(label_)
        , color(color_)
    {
    }
...
public:
    Button_widget(Device_base* device_ptr_, const Symbol& widget_name_, GU::Point location_, GU::Size size_,
        ...
        : Object_widget(device_ptr_, widget_name_, location_, size_)
        , label(label_)
        , on_color(on_color_)
...
{
public:
    Labeled_field_widget(Device_base* device_ptr_, const Symbol& widget_name_, GU::Point location_,
                         GU::Size label_size_, GU::Size field_size_, const Symbol& label_,
                         const Symbol& contents_ = Empty_string_c, const Symbol& color_ = Black_c);
