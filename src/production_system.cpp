...
void Production_System::clear()
{
    ...
    // this will start with pattern nodes and include them in the accumulated set, all combining nodes, and all rule
    // nodes
    ...
    for (Discrimination_Node_ptr_set_t::iterator it = discrimination_nodes.begin(); it != discrimination_nodes.end();
         it++) {
...
void Production_System::initialize()
{
    ...
    /*
    for(Clause_list_t::const_iterator init_it = initial_memory_state.begin(); init_it != initial_memory_state.end();
    init_it++) { if (!root_node_ptr->start_add_clause(*init_it)) cout << "No match: " << (*init_it) << endl;
            }
    */
}
...
bool Production_System::compile_file(string filename)
{
    ...
    */
    ...
    bool rule_set_is_good =
        parser.parse_file(production_list, initial_memory_state, named_locations, parameters, output_compiler_messages);
...
void Production_System::display_static_contents() const
{
    ...
    PPS_out << endl << "=============== STATIC CONTENTS ================" << endl;
    Assert(root_node_ptr);
    if (PPS_out) {
        PPS_out << "\n\nFinal Network:" << endl;
    ...
    for (Rule_Node_ptr_list_t::const_iterator it = rule_nodes.begin(); it != rule_nodes.end(); it++) {
        if (Combining_Node* p = dynamic_cast<Combining_Node*>(*it))
            try {
                ...
                PPS_out << "Unable to display contents of this combining_node (" << e.what() << ")" << endl;
            }
    }
...
void Production_System::display_dynamic_contents() const
{
    ...
    PPS_out << endl << "=============== DYNAMIC CONTENTS ================" << endl;
    for (Rule_Node_ptr_list_t::const_iterator it = rule_nodes.begin(); it != rule_nodes.end(); it++) {
        if (Combining_Node* p = dynamic_cast<Combining_Node*>(*it))
            try {
...
// The counter shows cycles completed; 0 during first cycle, 1 afterwards. If accessed before, shows one less than if
// accessed after
...
{
    Rule_name_match(const Symbol& probe_)
        : probe(probe_)
    {
    }
...
Rule_Node* Production_System::get_rule_node(const Symbol& name, bool throw_not_there) const
{
    ...
    Rule_Node_ptr_list_t::const_iterator it = find_if(rule_nodes.begin(), rule_nodes.end(), pred);
