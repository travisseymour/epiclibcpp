...
void Cognitive_processor::handle_event(const Cognitive_Update_event* event_ptr)
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << event_ptr->tag_name << " update: " << event_ptr->object_name << " "
                  << event_ptr->property_name << " from " << event_ptr->old_value << " to " << event_ptr->new_value
                  << endl;


    if (event_ptr->old_value != Nil_c)
        ps.update_delete_clause(
            Clause(event_ptr->tag_name, event_ptr->object_name, event_ptr->property_name, event_ptr->old_value));
    if (event_ptr->new_value != Nil_c)
        ps.update_add_clause(
            Clause(event_ptr->tag_name, event_ptr->object_name, event_ptr->property_name, event_ptr->new_value));
}
...
void Cognitive_processor::update_object_property(const Symbol& type_name, const Symbol& object_name,
                                                 const Symbol& property_name, const Symbol& old_value,
                                                 const Symbol& new_value)
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << type_name << " update: " << object_name << " " << property_name << " from "
                  << old_value << " to " << new_value << endl;
