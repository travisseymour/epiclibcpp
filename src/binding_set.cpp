...
bool Binding_set::operator==(const Binding_set& rhs) const
{
    ...
    for (lhs_it = binding_pair_list.begin(), rhs_it = rhs.binding_pair_list.begin();
         lhs_it != binding_pair_list.end() && rhs_it != rhs.binding_pair_list.end(); lhs_it++, rhs_it++)
        if (*lhs_it != *rhs_it)
            return false;
...
// New version avoiding the depreciated std::bind1st 08/17/25 tls
void Binding_set::add_binding_pair(const Binding_pair& bp)
{
    auto it = std::upper_bound(binding_pair_list.begin(), binding_pair_list.end(), bp, std::less<Binding_pair>{});
    binding_pair_list.insert(it, bp);
}
...
bool Binding_set::is_legal() const
{
    ...
    for (it = binding_pair_list.begin(), n_it = name_list.begin(); it != binding_pair_list.end() && n_it != n_end;
         it++, n_it++) {
