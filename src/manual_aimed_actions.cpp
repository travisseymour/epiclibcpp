...
std::shared_ptr<Motor_action> Manual_aimed_action::create(Human_processor* human_ptr, Symbol_list_t arguments,
...
Manual_aimed_action::Manual_aimed_action(Human_processor* human_ptr_, Symbol_list_t arguments,
...
Manual_aimed_action::Manual_aimed_action(Human_processor* human_ptr_, const Symbol& cursor_, const Symbol& target_,
                                         const Symbol& hand_, bool execute_when_prepared,
...
long Manual_aimed_action::prepare(long base_time)
{
    ...
    shared_ptr<Manual_aimed_action> exptr =
        std::dynamic_pointer_cast<Manual_aimed_action>(get_processor_ptr()->get_executing_ptr());
    ...
    int features = 0;
    shared_ptr<Manual_aimed_action> ptr =
        std::dynamic_pointer_cast<Manual_aimed_action>(get_processor_ptr()->get_previous_ptr());
    if (ptr) {
...
long Manual_aimed_action::execute(long base_time)
{
    ...
    Coordinator::get_instance().schedule_event(
        new Cognitive_Add_Clause_event(completion_time, cognitive_ptr, movement_description));
    Coordinator::get_instance().schedule_event(new Cognitive_Delete_Clause_event(
        completion_time + get_processor_ptr()->get_efferent_deletion_delay(), cognitive_ptr, movement_description));
...
long Manual_aimed_action::generate_movement_events(long base_time)
{
    ...
    movement_time +=
        long((get_processor_ptr()->auto_home_to_mouse()) * fluctuation); // right hand used on mouse or joystick
    ...
    if (Trace_out & get_trace()) {
        Trace_out << get_processor_ptr()->processor_info() << " Aimed movement of " << cursor << '/'
                  << cursor_device_name << " to " << target << '/' << target_device_name << endl;
    }
    ...
    for (int i = 0; i < number_of_subevents - 1; i++) {
        ...
        Coordinator::get_instance().schedule_event(new Device_Ply_event(base_time + i * subevent_time, get_device_ptr(),
                                                                        cursor_device_name, no_target, current_location,
                                                                        subevent_pv));
    }
    // send the last event with the exact target location
    Coordinator::get_instance().schedule_event(new Device_Ply_event(
        device_input_time, get_device_ptr(), cursor_device_name, target_device_name, target_location, movement_vector));
...
double Manual_aimed_action::modified_Welfords_Fitts_law(double d, double s, double minimum_time,
                                                        double coefficient) const
{
    double id = (d / s) + .5;
    double t = (id < 1.) ? 0. : coefficient * logb2(id); // log2 is C99 standard function
...
long Manual_Point_action::prepare(long base_time)
{
    ...
    shared_ptr<Manual_Point_action> ptr =
        std::dynamic_pointer_cast<Manual_Point_action>(get_processor_ptr()->get_previous_ptr());
    if (ptr) {
        // dk 090807 - no r, theta feature programming time cost, but charge for a change in style or hand
...
double Manual_Point_action::determine_movement_time() const
{
    ...
    if (target_size == GU::Size()) {
        ...
        movement_time = double(KLM_point_time - 2 * get_processor_ptr()->get_feature_time() -
                               get_processor_ptr()->get_initiation_time());
    }
    else {
        long coefficient =
            (get_processor_ptr()->get_button_held())
...
Manual_Click_on_action::Manual_Click_on_action(Human_processor* human_ptr_, Symbol_list_t arguments,
                                               bool execute_when_prepared)
    : Manual_Point_action(human_ptr_, arguments, execute_when_prepared, "Motor Manual Click_on")
    , keyname(Mouse_Left_Button_c)
{
}
...
long Manual_Click_on_action::prepare(long base_time)
{
    ...
    long feature_time = get_processor_ptr()->get_feature_time();
    shared_ptr<Manual_Click_on_action> ptr =
        std::dynamic_pointer_cast<Manual_Click_on_action>(get_processor_ptr()->get_previous_ptr());
    // dk 090807 - no r, theta feature programming time cost, but charge for a change in style or hand
    if (!ptr) {
...
long Manual_Click_on_action::generate_movement_events(long base_time)
{
    ...
    Coordinator::get_instance().schedule_event(
        new Device_Keystroke_event(device_input_time, get_device_ptr(), keyname));
