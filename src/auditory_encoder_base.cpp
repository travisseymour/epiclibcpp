...
void Auditory_encoder_base::schedule_change_property_event(long recoding_time, const Symbol& obj_name,
                                                           const Symbol& propname, const Symbol& encoded_property_value)
{
    perceptual_proc_ptr->schedule_event(new Auditory_Sound_Set_Property_event(
        get_time() + recoding_time, perceptual_proc_ptr->get_human_ptr()->get_visual_perceptual_store_ptr(), obj_name,
        propname, encoded_property_value));
}
...
void Auditory_encoder_base::schedule_delayed_delivery_event(long delay_time, const Symbol& object_name,
                                                            const Symbol& property_name, const Symbol& property_value)
{
    ...
    perceptual_proc_ptr->schedule_event(
        new Auditory_Delay_event(event_time, perceptual_proc_ptr, object_name, property_name, property_value));
}
...
void Auditory_encoder_base::schedule_delay_event(long delay, const Symbol& object_name, const Symbol& property_name,
                                                 const Symbol& property_value)
{
    if (Trace_out && perceptual_proc_ptr->get_trace())
        Trace_out << processor_info() << " Schedule delay of " << delay << ' ' << object_name << ' ' << property_name
                  << ' ' << property_value << endl;
    Event* output_event_ptr =
        new Auditory_Delay_event(get_time() + delay, perceptual_proc_ptr, object_name, property_name, property_value);
