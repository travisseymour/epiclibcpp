...
public:
    ...
    bool parse_file(Compiler_rule_data_list_t& production_list, Clause_list_t& initial_memory_contents,
                    ...
                    Parameter_specification_list_t& parameters, bool output_compiler_messages);
...
private:
    ...
    void parse_named_location(std::map<Symbol, Geometry_Utilities::Point>& named_locations,
    ...
    Symbol_list_t parse_command(); // combines parse_motor_command() and parse_log_command(), which had identical code.
                                   // ajh & teh - 3/08
    ...
    struct Arg_requirements
    {
        ...
        Arg_requirements(int in_num_required_args = 0, int in_num_required_vars = 0)
            : num_required_args(in_num_required_args)
            , num_required_vars(in_num_required_vars)
        {
        }
    };
...
{
public:
    PPS_parser_exception()
        : PPS_exception("Unspecified PPS Parser exception")
    {
    }
    PPS_parser_exception(const std::string& msg)
        : PPS_exception(msg)
    {
    }
