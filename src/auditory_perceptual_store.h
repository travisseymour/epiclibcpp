...
{
public:
    //	static Smart_Pointer<Auditory_sound> create(const Symbol& sound_name, const Symbol& stream_name, long
    // time_stamp, const Symbol& timbre,
    static std::shared_ptr<Auditory_sound> create(const Symbol& sound_name, const Symbol& stream_name, long time_stamp,
                                                  GU::Point location, const Symbol& timbre, double loudness,
                                                  long duration, long intrinsic_duration = 0)
    {
        return std::shared_ptr<Auditory_sound>(new Auditory_perceptual_sound(
            sound_name, stream_name, time_stamp, location, timbre, loudness, duration, intrinsic_duration));
    }
    // fatter interface for speech sounds
    //	static std::shared_ptr<Auditory_sound> create(const Symbol& sound_name, const Symbol& stream_name, long
    // time_stamp, GU::Point location, 		const Symbol& timbre, 		const Symbol& content, const Symbol& gender,
    // const Symbol& speaker_id, int utterance_id, 		double loudness, double pitch, long duration, long
    // intrinsic_duration = 0) 			{return new Auditory_perceptual_sound(sound_name, stream_name, time_stamp,
    // timbre, content, gender, speaker_id, loudness, duration, intrinsic_duration);} 			{return
    // std::shared_ptr<Auditory_sound>(new Auditory_perceptual_sound(sound_name, stream_name, time_stamp, location,
    // timbre, content, gender, speaker_id, utterance_id, loudness, pitch, duration, intrinsic_duration));}
    ...
    enum Auditory_perceptual_sound_e
    {
        PRESENT,
        UNSUPPORTED,
        RETAINED
