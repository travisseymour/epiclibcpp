/*
This serves as the base class for a custom encoder object that the Visual_perceptual_processor forwards
...
#ifndef VISUAL_ENCODER_BASE_H
#define VISUAL_ENCODER_BASE_H
...
public:
    ...
    Visual_encoder_base(const std::string& id);
    //	virtual ~Visual_encoder_base() = 0;	// abstract class
    virtual ~Visual_encoder_base(){}; // ** NON ** abstract class for use with PYBIND11
    ...
    // must be called after construction to connect to the Visual_perceptual_processor
    void connect(Visual_perceptual_processor* perceptual_proc_ptr_)
    {
        perceptual_proc_ptr = perceptual_proc_ptr_;
    }

    Visual_perceptual_processor* get_perceptual_proc_ptr() const
    {
    ...
    */
    ...
    virtual bool set_object_property(const Symbol& object_name, const Symbol& property_name,
                                     const Symbol& property_value, long encoding_time);
    virtual bool handle_Delay_event(const Symbol& object_name, const Symbol& property_name,
                                    const Symbol& property_value);

    // ======= TLS April 23: Moved this here so I could create EPICPyVisualEncoder
    // schedule an event to be sent to the Visual_perceptual_store at current time + recoding time
    void schedule_change_property_event(long recoding_time, const Symbol& object_name, const Symbol& property_name,
                                        const Symbol& property_value);
...
protected:
    ...
    long get_time() const;
    // get the trace state of the associated Visual_perceptual_processor
    ...
    // access the size or location for the named object
    Geometry_Utilities::Point get_object_location(const Symbol& object_name) const;
    Geometry_Utilities::Size get_object_size(const Symbol& object_name) const;
    ...
    // schedule a delayed and cancelable delivery of a property for an object to happen at current time + delay time,
    // cancelable before then
    void schedule_delayed_delivery_event(long delay_time, const Symbol& object_name, const Symbol& property_name,
                                         const Symbol& property_value);
    ...
    void schedule_delay_event(long delay, const Symbol& object_name, const Symbol& property_name,
                              const Symbol& property_value);

private:
    std::string encoder_name;
    Visual_perceptual_processor* perceptual_proc_ptr; // the connected device processor interface to system

    Visual_encoder_base(const Visual_encoder_base&);
    Visual_encoder_base& operator=(const Visual_encoder_base&);
};
