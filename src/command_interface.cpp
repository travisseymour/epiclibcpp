...
void Command_interface::dispatch(const Symbol_list_t& arguments)
{
    ...
    // added by ajh & teh 2 - 3/17/08
        ...
        if (args.front() == Start_c)
            ...
            throw Command_exception(human_ptr, string("Invalid rule Temporal action specification: (") +
                                                   concatenate_to_string(arguments) + ")");
        return;
    }
    ...
    // Modulate attentional focus
        ...
        if (args.front() == Focus_c)
            ...
            throw Command_exception(human_ptr, string("Invalid rule Modulate attention: (") +
                                                   concatenate_to_string(arguments) + ")");
        return;
    }
    ...
    {
        throw Command_exception(human_ptr, string("Invalid rule action specification: (") +
                                               concatenate_to_string(arguments) + ")");
    }
}
...
bool Command_interface::dispatch(const Symbol_list_t& arguments, const Symbol_list_t& pattern)
{
    if (arguments.front() == "Add_with_probability") {
        ...
        if (prob_param.has_numeric_value()) {
            ...
            if (prob < 0 || prob > 1.0)
                throw Command_exception(human_ptr, string("Invalid Add_with_probability specification: (") +
                                                       concatenate_to_string(arguments) + ")");
        }
