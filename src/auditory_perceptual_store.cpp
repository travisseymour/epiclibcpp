...
{
    {return std::shared_ptr<Auditory_sound>(new Auditory_perceptual_sound(word.name, word.stream_name, word.time_stamp,
word.location, Speech_c, word.content, word.speaker_gender, word.speaker_id, word.utterance_id, word.loudness,
word.pitch, 0, word.duration));}
}
*/
...
{
    shared_ptr<Auditory_perceptual_sound> obj_ptr =
        dynamic_pointer_cast<Auditory_perceptual_sound>(get_sound_ptr(sound_name));
...
void Auditory_perceptual_store::handle_event(const Auditory_Stream_Create_event* event_ptr)
{
    ...
    if (changed) {
        ...
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->stream_name,
                                                                               Status_c, Nil_c, Present_c);
    }
}
...
void Auditory_perceptual_store::handle_event(const Auditory_Stream_Destroy_event* event_ptr)
{
    ...
    schedule_event(new Auditory_Stream_Erase_event(get_time() + unsupported_decay_delay.get_long_value(), this,
...
void Auditory_perceptual_store::handle_event(const Auditory_Stream_Erase_event* event_ptr)
{
    ...
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, stream_name, Status_c, Present_c,
                                                                           Nil_c);
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, stream_name, Status_c, Present_c,
                                                                           Nil_c);
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, stream_name, Status_c, Present_c,
    ...
    for (; it_name != prop_names.end() && it_value != prop_values.end(); ++it_name, ++it_value) {
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, stream_name, *it_name,
                                                                               *it_value, Nil_c);
    }
...
void Auditory_perceptual_store::handle_event(const Auditory_Stream_Set_Location_event* event_ptr)
{
    ...
    if (changed) {
        ...
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->stream_name,
                                                                               Location_c, Nil_c, Change_c);
        // schedule for later delivery
        schedule_event(new Cognitive_Update_event(get_time() + change_decay_time.get_long_value(),
                                                  get_human_ptr()->get_Cognitive_processor_ptr(), Auditory_c,
                                                  event_ptr->stream_name, Location_c, Change_c, Nil_c));
    }
}
...
void Auditory_perceptual_store::handle_event(const Auditory_Stream_Set_Pitch_event* event_ptr)
{
    ...
    if (changed) {
        // immediately post the change
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->stream_name,
                                                                               Pitch_c, Nil_c, Change_c);
        // schedule for later delivery
        schedule_event(new Cognitive_Update_event(get_time() + change_decay_time.get_long_value(),
                                                  get_human_ptr()->get_Cognitive_processor_ptr(), Auditory_c,
                                                  event_ptr->stream_name, Pitch_c, Change_c, Nil_c));
    }
}
...
void Auditory_perceptual_store::handle_event(const Auditory_Stream_Set_Loudness_event* event_ptr)
{
    ...
    if (changed) {
        // immediately post the change
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->stream_name,
                                                                               Loudness_c, Nil_c, Change_c);
        // schedule for later delivery
        schedule_event(new Cognitive_Update_event(get_time() + change_decay_time.get_long_value(),
                                                  get_human_ptr()->get_Cognitive_processor_ptr(), Auditory_c,
                                                  event_ptr->stream_name, Loudness_c, Change_c, Nil_c));
    }
}
...
void Auditory_perceptual_store::handle_event(const Auditory_Stream_Set_Size_event* event_ptr)
{
    ...
    if (changed) {
        // immediately post the change
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->stream_name,
                                                                               Size_c, Nil_c, Change_c);

        // schedule for later delivery
        schedule_event(new Cognitive_Update_event(get_time() + change_decay_time.get_long_value(),
                                                  get_human_ptr()->get_Cognitive_processor_ptr(), Auditory_c,
                                                  event_ptr->stream_name, Size_c, Change_c, Nil_c));
    }
}
...
void Auditory_perceptual_store::handle_event(const Auditory_Stream_Set_Property_event* event_ptr)
{
    ...
    Symbol old_value = Auditory_store::set_stream_property(event_ptr->stream_name, event_ptr->property_name,
                                                           event_ptr->property_value);
    // send through to cognitive processor immediately
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(
        Auditory_c, event_ptr->stream_name, event_ptr->property_name, old_value, event_ptr->property_value);
}
...
void Auditory_perceptual_store::handle_event(const Auditory_Sound_Start_event* event_ptr)
{
    ...
    insert_new(Auditory_perceptual_sound::create(event_ptr->sound_name, event_ptr->stream_name, event_ptr->time_stamp,
                                                 event_ptr->location, event_ptr->timbre, 0,
                                                 event_ptr->intrinsic_duration));

    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "start sound " << event_ptr->sound_name << event_ptr->stream_name << ' '
                  << event_ptr->time_stamp << ' ' << event_ptr->timbre << ' ' << event_ptr->intrinsic_duration << endl;

    notify_views(&View_base::notify_auditory_sound_start, event_ptr->sound_name, event_ptr->stream_name,
                 event_ptr->time_stamp, event_ptr->location, event_ptr->timbre, event_ptr->loudness);

    changed = true;


    // immediately post that the object is audible and its timbre and time_stamp
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->sound_name, Status_c,
                                                                           Nil_c, Audible_c);
    // create a stream clause only if one was specified:
    if (event_ptr->stream_name != Symbol())
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->sound_name,
                                                                               Stream_c, Nil_c, event_ptr->stream_name);
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->sound_name, Timbre_c,
                                                                           Nil_c, event_ptr->timbre);
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->sound_name,
                                                                           Time_stamp_c, Nil_c, event_ptr->time_stamp);
}
...
void Auditory_perceptual_store::handle_event(const Auditory_Speech_Start_event* event_ptr)
{
    ...
    // create the new sound object - a continuing sound has duration 0
    //	insert_new(Auditory_perceptual_sound::create(event_ptr->sound_name, event_ptr->stream_name,
    // event_ptr->time_stamp, 		Speech_c, event_ptr->content,  event_ptr->speaker_gender,  event_ptr->speaker_id,
    // event_ptr->loudness, 0, event_ptr->duration));

    insert_new(Auditory_perceptual_sound::create(event_ptr->word));

    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "start speech sound " << event_ptr->word.name << event_ptr->word.stream_name
                  << ' ' << event_ptr->word.time_stamp << ' ' << event_ptr->word.location << ' '
                  << event_ptr->word.content << ' ' << event_ptr->word.speaker_gender << ' '
                  << event_ptr->word.speaker_id << ' ' << event_ptr->word.loudness << ' ' << event_ptr->word.pitch
                  << ' ' << event_ptr->word.duration << endl;

    //	notify_views(&View_base::notify_auditory_speech_start, event_ptr->sound_name, event_ptr->stream_name,
    // event_ptr->time_stamp, event_ptr->content, event_ptr->speaker_gender, event_ptr->speaker_id,
    // event_ptr->loudness);
    ...
    // immediately post that the object is audible and its timbre and time_stamp
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->word.name, Status_c,
                                                                           Nil_c, Audible_c);
    // create a stream clause only if one was specified:
    if (event_ptr->word.stream_name != Symbol())
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(
            Auditory_c, event_ptr->word.name, Stream_c, Nil_c, event_ptr->word.stream_name);
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(
        Auditory_c, event_ptr->word.name, Time_stamp_c, Nil_c, event_ptr->word.time_stamp);
    // provide only the azimuth (location x-value)
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->word.name, Azimuth_c,
                                                                           Nil_c, event_ptr->word.location.x);
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->word.name, Content_c,
                                                                           Nil_c, event_ptr->word.content);
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->word.name, Gender_c,
                                                                           Nil_c, event_ptr->word.speaker_gender);
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->word.name, Speaker_c,
                                                                           Nil_c, event_ptr->word.speaker_id);
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->word.name, Loudness_c,
                                                                           Nil_c, event_ptr->word.loudness);
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, event_ptr->word.name, Pitch_c,
                                                                           Nil_c, event_ptr->word.pitch);
}
...
void Auditory_perceptual_store::handle_event(const Auditory_Sound_Stop_event* event_ptr)
{
    ...
    // The object must first go into the unsupported state - it is still fully present as far as anyone else is
    // concerned Put the object into the "Unsupported" state to show that perceptual input no longer supports it
    obj_ptr->set_state(Auditory_perceptual_sound::UNSUPPORTED);

    // schedule an erasure event for later
    schedule_event(new Auditory_Sound_Erase_event(get_time() + unsupported_decay_delay.get_long_value(), this,
...
void Auditory_perceptual_store::handle_event(const Auditory_Sound_Erase_event* event_ptr)
{
    ...
    if (!is_sound_present(event_ptr->sound_name)) {
        if (get_trace() && Trace_out)
            Trace_out << processor_info() << event_ptr->sound_name << " no longer present for erasure" << endl;
        return;
    }
    ...
    case Auditory_perceptual_sound::UNSUPPORTED: {
        ...
        // schedule an erasure event for later
        schedule_event(new Auditory_Sound_Erase_event(get_time() + retained_decay_delay.get_long_value(), this,
                                                      event_ptr->sound_name));
...
void Auditory_perceptual_store::lose_sound_from_memory(shared_ptr<Auditory_perceptual_sound> obj_ptr)
{
    ...
    case Auditory_perceptual_sound::UNSUPPORTED: {
        ...
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, sound_name, Status_c,
                                                                               Audible_c, Nil_c);
        // deliberate drop through to the RETAINED case
    }
    case Auditory_perceptual_sound::RETAINED: {
        // remove stream, timbre and next separately, and also speech characteristics
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, sound_name, Timbre_c,
                                                                               obj_ptr->get_timbre(), Nil_c);
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, sound_name, Stream_c,
                                                                               obj_ptr->get_stream(), Nil_c);
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, sound_name, Time_stamp_c,
                                                                               obj_ptr->get_time_stamp(), Nil_c);
        // remove only the azimuth (location x-value)
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, sound_name, Azimuth_c,
                                                                               obj_ptr->get_location().x, Nil_c);
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, sound_name, Content_c,
                                                                               obj_ptr->get_content(), Nil_c);
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, sound_name, Gender_c,
                                                                               obj_ptr->get_gender(), Nil_c);
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, sound_name, Speaker_c,
                                                                               obj_ptr->get_speaker_id(), Nil_c);
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, sound_name, Loudness_c,
                                                                               obj_ptr->get_loudness(), Nil_c);
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, sound_name, Pitch_c,
                                                                               obj_ptr->get_pitch(), Nil_c);
    ...
    Symbol_list_t ::iterator it_value = prop_values.begin();
    for (; it_name != prop_names.end() && it_value != prop_values.end(); ++it_name, ++it_value) {
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(Auditory_c, sound_name, *it_name,
                                                                               *it_value, Nil_c);
    }
...
void Auditory_perceptual_store::handle_event(const Auditory_Sound_Set_Property_event* event_ptr)
{
    ...
    if (event_ptr->property_value == Nil_c && old_value != Nil_c && event_ptr->property_name != Detection_c) {
        if (get_trace() && Trace_out)
            Trace_out << processor_info() << event_ptr->sound_name << " losing perceptual support for "
        ...
        schedule_event(new Auditory_Sound_Erase_Property_event(disappearing_time, this, event_ptr->sound_name,
                                                               event_ptr->property_name, old_value));
    ...
    // send through to cognitive processor immediately
    get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(
        Auditory_c, event_ptr->sound_name, event_ptr->property_name, old_value, event_ptr->property_value);
}
...
void Auditory_perceptual_store::handle_event(const Auditory_Sound_Erase_Property_event* event_ptr)
{
    ...
    // do nothing if the object is not present - not very efficient, but erasure of whole object may have already
    // happened
    if (!is_sound_present(event_ptr->sound_name)) {
        if (get_trace() && Trace_out)
            Trace_out << processor_info() << event_ptr->sound_name << " no longer present for erasure of "
                      << event_ptr->property_name << endl;
        return;
    }
    ...
    if (get_time() == disappearing_time && current_value == event_ptr->property_value) {
        if (get_trace() && Trace_out)
            Trace_out << processor_info() << event_ptr->sound_name << " property " << event_ptr->property_name
                      << " lost" << endl;
        // update the store and broadcast the change
        Symbol old_value = Auditory_store::set_sound_property(event_ptr->sound_name, event_ptr->property_name, Nil_c);
        // send through to cognitive processor immediately
        get_human_ptr()->get_Cognitive_processor_ptr()->update_object_property(
            Auditory_c, event_ptr->sound_name, event_ptr->property_name, event_ptr->property_value, Nil_c);
    }
