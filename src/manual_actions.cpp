...
shared_ptr<Motor_action> Manual_action::create(Human_processor* human_ptr, Symbol_list_t arguments,
...
{
    human_ptr->schedule_event(new Motor_Command_Action_event(time, human_ptr->get_manual_ptr(), shared_from_this()));
}
...
Manual_Keystroke_action::Manual_Keystroke_action(Human_processor* hum_ptr, Symbol_list_t arguments,
                                                 bool execute_when_prepared)
...
long Manual_Keystroke_action::prepare(long base_time)
{
    ...
    shared_ptr<Manual_Keystroke_action> ptr =
        std::dynamic_pointer_cast<Manual_Keystroke_action>(get_processor_ptr()->get_previous_ptr());
...
long Manual_Keystroke_action::execute(long base_time)
{
    ...
    long keystroke_execution_time =
        get_processor_ptr()->get_parameter_ptr("KLM_keystroke_execution_time")->get_long_value();
    ...
    Coordinator::get_instance().schedule_event(
        new Device_Keystroke_event(movement_completion_time, get_device_ptr(), keyname));
...
{
    long feature_preparation_time;
    shared_ptr<Manual_Hold_action> ptr =
        std::dynamic_pointer_cast<Manual_Hold_action>(get_processor_ptr()->get_previous_ptr());
    long feature_time = get_processor_ptr()->get_feature_time();
...
{
    double fluctuation = get_processor_ptr()->get_execution_fluctuation();
    long hold_execution_time =
        get_processor_ptr()->get_parameter_ptr("KLM_hold_release_execution_time")->get_long_value();
    ...
    Coordinator::get_instance().schedule_event(
        new Device_Hold_event(movement_completion_time, get_device_ptr(), keyname));
...
{
    long feature_preparation_time;
    shared_ptr<Manual_Release_action> ptr =
        std::dynamic_pointer_cast<Manual_Release_action>(get_processor_ptr()->get_previous_ptr());
    long feature_time = get_processor_ptr()->get_feature_time();
...
{
    shared_ptr<Manual_Punch_action> ptr =
        std::dynamic_pointer_cast<Manual_Punch_action>(get_processor_ptr()->get_previous_ptr());
    int features;
...
long Manual_Punch_action::execute(long base_time)
{
    ...
    Coordinator::get_instance().schedule_event(
        new Cognitive_Add_Clause_event(downstroke_done_time, cognitive_ptr, movement_description));
    Coordinator::get_instance().schedule_event(
        new Cognitive_Delete_Clause_event(downstroke_done_time + get_processor_ptr()->get_efferent_deletion_delay(),
