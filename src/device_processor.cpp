...
Device_processor::Device_processor(const std::string& id)
    : Processor(id)
    , device_interface_ptr(0)
    , human_interface_ptr(0)
{
}
...
void Device_processor::accept_event(const Start_event*)
{
    Assert(device_interface_ptr);
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Asking Device to call its handle_Start_event() method" << "("
                  << typeid(device_interface_ptr).name() << ")" << endl;
...
void Device_processor::accept_event(const Stop_event*)
{
    Assert(device_interface_ptr);
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Asking Device to call its handle_Stop_event() method" << "("
                  << typeid(device_interface_ptr).name() << ")" << endl;
...
void Device_processor::handle_event(const Device_Delay_event* event_ptr)
{
    ...
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Delay " << event_ptr->type << ' ' << event_ptr->datum << ' '
                  ...
                  << endl;
    device_interface_ptr->handle_Delay_event(event_ptr->type, event_ptr->datum, event_ptr->object_name,
                                             event_ptr->property_name, event_ptr->property_value);
}
...
void Device_processor::handle_event(const Device_Ply_event* event_ptr)
{
    ...
    device_interface_ptr->handle_Ply_event(event_ptr->cursor_name, event_ptr->target_name, event_ptr->new_location,
                                           event_ptr->movement_vector);
}
...
void Device_processor::handle_event(const Device_Eyemovement_Start_event* event_ptr)
{
    Assert(device_interface_ptr);
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Eyemovement started to: " << event_ptr->target_name << ' '
                  << event_ptr->new_location << endl;
...
void Device_processor::handle_event(const Device_Eyemovement_End_event* event_ptr)
{
    Assert(device_interface_ptr);
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Eyemovement ended at: " << event_ptr->target_name << ' '
                  << event_ptr->new_location << endl;
...
void Device_processor::schedule_delay_event(long delay)
{
    ...
    Event* output_event_ptr = new Device_Delay_event(get_time() + delay, this);
...
{
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Schedule delay of " << delay << ' ' << delay_type << ' ' << delay_datum
                  << endl;
    Event* output_event_ptr = new Device_Delay_event(get_time() + delay, this, delay_type, delay_datum);

    schedule_event(output_event_ptr);
}

void Device_processor::schedule_delay_event(long delay, const Symbol& delay_type, const Symbol& object_name,
                                            const Symbol& property_name, const Symbol& property_value)
{
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Schedule delay of " << delay << ' ' << delay_type << ' ' << object_name
                  << ' ' << property_name << ' ' << property_value << endl;
    Event* output_event_ptr =
        new Device_Delay_event(get_time() + delay, this, delay_type, object_name, property_name, property_value);
...
void Device_processor::make_visual_object_appear(const Symbol& object_name, Geometry_Utilities::Point location,
                                                 Geometry_Utilities::Size size)
{
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Make appear: " << object_name << " at " << location << " size " << size
                  << endl;
...
{
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Set location: " << object_name << " to " << location << endl;
...
{
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Set size: " << object_name << " to " << size << endl;
...
void Device_processor::set_visual_object_property(const Symbol& object_name, const Symbol& property_name,
                                                  const Symbol& property_value)
{
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Set: " << object_name << ' ' << property_name << ' ' << property_value
                  << endl;
...
{
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Set location: " << name << " to " << location << endl;
...
{
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Set size: " << name << " to " << size << endl;
...
{
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Set: " << name << ' ' << propname << ' ' << propvalue << endl;
...
void Device_processor::make_auditory_sound_event(const Symbol& name, const Symbol& stream, GU::Point location,
                                                 const Symbol& timbre, double loudness, long duration,
                                                 long intrinsic_duration)
{
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Sound event: " << name << ' ' << stream << ' ' << location << ' ' << timbre
                  << ' ' << loudness << ' ' << duration << ' ' << intrinsic_duration << endl;
    human_interface_ptr->make_auditory_sound_event(name, stream, location, timbre, loudness, duration,
                                                   intrinsic_duration);
}

// A new auditory sound with timbre, stream, and instrinsic duration has started
void Device_processor::make_auditory_sound_start(const Symbol& name, const Symbol& stream, GU::Point location,
                                                 const Symbol& timbre, double loudness, long intrinsic_duration)
{
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Sound start: " << name << ' ' << stream << ' ' << location << ' ' << timbre
...
{
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Sound stop: " << name << endl;
        ...
        const Symbol& content, const Symbol& speaker_gender, const Symbol& speaker_id, double loudness, long duration)
{
    if(Trace_out && get_trace())
        Trace_out << processor_info() << " Speech sound event: "
            << sound_name << ' ' << stream_name << ' ' << content << ' ' << speaker_gender << ' ' << speaker_id << ' '
<< loudness << ' ' << duration << endl; human_interface_ptr->make_auditory_speech_event(sound_name, stream_name,
content, speaker_gender, speaker_id, loudness, duration);
}
*/
void Device_processor::make_auditory_speech_event(const Speech_word& word)
{
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Speech sound event: " << word.name << ' ' << word.stream_name << ' '
                  << word.content << ' ' << word.speaker_gender << ' ' << word.speaker_id << ' ' << word.pitch << ' '
                  << word.loudness << ' ' << word.duration << endl;
...
void Device_processor::make_high_level_input_appear(const Symbol& object_name, const Symbol_list_t& props,
                                                    const Symbol_list_t& values, const Symbol& tag)
{
    if (Trace_out && get_trace()) {
...
void Device_processor::make_report(long time, long duration)
{
    ...
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Schedule report event at time: " << time << " with duration: " << duration
                  << endl;
    human_interface_ptr->make_report(duration);
}

// Allow the device to set a human parameter - for generating parametric runs
void Device_processor::set_human_parameter(const std::string& proc_name, const std::string& param_name,
                                           const std::string& spec)
{
    if (Trace_out && get_trace())
        Trace_out << processor_info() << " Set human parameter: " << proc_name << ' ' << param_name << ' ' << spec
                  << endl;
