...
void Stream_tracker_MinDistNoisy::assign_sounds_to_streams()
{
    ...
    if ((max_total_dist - min_total_dist) <= theta) {
        ...
        //      do {
        //           random_shuffle(min_total_distance_assignment.begin(), min_total_distance_assignment.end());
        //           shuffle(min_total_distance_assignment.begin(), min_total_distance_assignment.end(),
        //           get_Random_engine()); } while (assignments == min_total_distance_assignment);
    }
    // obsolete 10/27/14
    ...
    for (int stream_index = 0; stream_index < min_total_distance_assignment.size(); stream_index++) {
        ...
        /*       if(streams[stream_index]->get_last_veridical_stream() != sounds[sound_index]->get_stream()) {
                    Normal_out << "Stream switch: Stream, utterance for "
                        ...
                        << streams[stream_index]->get_last_veridical_stream() << ' ' <<
           streams[stream_index]->get_last_utterance_id()
                        << " now " << sounds[sound_index]->get_stream() << ' ' <<
           sounds[sound_index]->get_utterance_id()
        ...
        //       Normal_out << "sound " << sound_index << ' ' << sounds[sound_index]->get_utterance_id() << " assigned
        //       to stream " << stream_index << endl;
    }
    /*

        double distances[max_n_sounds_present][max_n_sounds_present]; // first index is sound index, second is stream
       index
...
double Stream_tracker_MinDistNoisy::get_total_distance(vector<int> assignments) const
{
    ...
    for (int stream_index = 0; stream_index < assignments.size(); stream_index++) {
        ...
        total += get_distance(sounds[sound_index]->get_pitch(), sounds[sound_index]->get_loudness(),
...
double Stream_tracker_MinDistNoisy::get_distance(double pitch, double loudness, double azimuth,
                                                 std::shared_ptr<Stream_base> stream) const
{
    //	return get_distance(pitch, loudness, stream.get_pitch_mean(), stream.get_loudness_mean());
    return get_distance(pitch, loudness, azimuth, stream->get_predicted_pitch(), stream->get_predicted_loudness(),
                        stream->get_predicted_azimuth());
}
...
double Stream_tracker_MinDistNoisy::get_distance(double sound_pitch, double sound_loudness, double sound_azimuth,
                                                 double stream_pitch, double stream_loudness,
                                                 double stream_azimuth) const
{
    double pitch_diff =
        fabs(sound_pitch -
             stream_pitch); // difference in semitones  range from +/- 0 to around 13 between sexes, could be more
    ...
    /*    // convert the sound azimuth to a z-score using the stream mean azimuth and the sd for that mean
        double azimuth_diff = fabs((sound_azimuth - stream_azimuth) /
       Ear_processor::get_auditory_location_sd(stream_azimuth));
    ...
    */
    ...
    //		Normal_out << pitch_diff << ' ' << loudness_diff << ' ' << azimuth_diff << ' ' << ((sum > 0.0) ? sqrt(sum) :
    // 0.0) << endl;
