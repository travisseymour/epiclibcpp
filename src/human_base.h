...
public:
    ...
    virtual void make_auditory_sound_event(const Symbol& name, const Symbol& stream, GU::Point location,
    ...
    virtual void make_auditory_sound_start(const Symbol& name, const Symbol& stream, GU::Point location,
    ...
    virtual void make_high_level_input_appear(const Symbol& object_name, const Symbol_list_t& props,
    ...
    virtual std::string get_prs_filename() const
    {
        return std::string();
    }
...
protected:
    Human_base()
    {
    } // this class serves only as a base for derived classes
    virtual ~Human_base()
    {
    }
};
