...
// accept each event, update the local store generically, and then call the perceptual processor if a change was made
...
void Auditory_sensory_store::handle_event(const Auditory_Stream_Create_event* event_ptr)
{
    ...
    get_human_ptr()->get_Auditory_perceptual_processor_ptr()->create_stream(event_ptr->stream_name, event_ptr->pitch,
                                                                            event_ptr->loudness, event_ptr->location);
}
...
void Auditory_sensory_store::handle_event(const Auditory_Stream_Destroy_event* event_ptr)
{
    ...
    schedule_event(new Auditory_Stream_Erase_event(get_time() + disappearance_decay_delay.get_long_value(), this,
...
void Auditory_sensory_store::handle_event(const Auditory_Stream_Set_Location_event* event_ptr)
{
    Auditory_store::handle_event(event_ptr);
    if (changed)
        get_human_ptr()->get_Auditory_perceptual_processor_ptr()->set_stream_location(event_ptr->stream_name,
                                                                                      event_ptr->location);
}
...
void Auditory_sensory_store::handle_event(const Auditory_Stream_Set_Pitch_event* event_ptr)
{
    Auditory_store::handle_event(event_ptr);
    if (changed)
        get_human_ptr()->get_Auditory_perceptual_processor_ptr()->set_stream_pitch(event_ptr->stream_name,
                                                                                   event_ptr->pitch);
}
...
void Auditory_sensory_store::handle_event(const Auditory_Stream_Set_Loudness_event* event_ptr)
{
    Auditory_store::handle_event(event_ptr);
    if (changed)
        get_human_ptr()->get_Auditory_perceptual_processor_ptr()->set_stream_loudness(event_ptr->stream_name,
                                                                                      event_ptr->loudness);
}
...
void Auditory_sensory_store::handle_event(const Auditory_Stream_Set_Size_event* event_ptr)
{
    Auditory_store::handle_event(event_ptr);
    if (changed)
        get_human_ptr()->get_Auditory_perceptual_processor_ptr()->set_stream_size(event_ptr->stream_name,
                                                                                  event_ptr->size);
}
...
void Auditory_sensory_store::handle_event(const Auditory_Stream_Set_Property_event* event_ptr)
{
    Auditory_store::handle_event(event_ptr);
    if (changed)
        get_human_ptr()->get_Auditory_perceptual_processor_ptr()->set_stream_property(
            event_ptr->stream_name, event_ptr->property_name, event_ptr->property_value);
}
...
{
    Auditory_store::handle_event(event_ptr);
    get_human_ptr()->get_Auditory_perceptual_processor_ptr()->make_new_sound_start(
        event_ptr->sound_name, event_ptr->stream_name, event_ptr->time_stamp, event_ptr->location, event_ptr->timbre,
        event_ptr->loudness, event_ptr->intrinsic_duration);
}
...
{
    Auditory_store::handle_event(event_ptr);
    get_human_ptr()->get_Auditory_perceptual_processor_ptr()->make_new_speech_start(event_ptr->word);
}
...
void Auditory_sensory_store::handle_event(const Auditory_Sound_Stop_event* event_ptr)
{
    ...
    schedule_event(new Auditory_Sound_Erase_event(get_time() + disappearance_decay_delay.get_long_value(),
                                                  get_human_ptr()->get_auditory_sensory_store_ptr(),
...
void Auditory_sensory_store::handle_event(const Auditory_Sound_Set_Property_event* event_ptr)
{
    Auditory_store::handle_event(event_ptr);
    if (changed)
        get_human_ptr()->get_Auditory_perceptual_processor_ptr()->set_sound_property(
            event_ptr->sound_name, event_ptr->property_name, event_ptr->property_value);
}