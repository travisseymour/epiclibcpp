...
void Ear_processor::set_parameter(const Parameter_specification& param_spec)
{
    /*	if(param_spec.parameter_name == "Content_detection") {
       ...
       param_spec); detection_parameters[cat_name] = Detection{mean, sd, lapse_rate};
            }
                                         ...
                                         long intrinsic_duration)
{
    ...
    schedule_event(new Auditory_Sound_Start_event(get_time() + appearance_window.get_long_value(), this,
                                                  psychological_name, stream_name, time_stamp, location, timbre,
                                                  loudness, intrinsic_duration));
    ...
    */
    ...
    //		Smart_Pointer<Motor_action> action_ptr = new Invol_ocular_Saccade_action(get_human_ptr(),
    // psychological_name); 		action_ptr->dispatch(get_time() + appearance_delay.get_value() +
    // inform_invol_ocular_delay.get_value());
    //		}
}
...
void Ear_processor::make_new_sound_start_f(const Symbol& psychological_name, const Symbol& stream_name, long time_stamp,
                                           GU::Point original_location, const Symbol& timbre, double loudness,
                                           long intrinsic_duration)
{
    if (get_trace() && Trace_out) {
    ...
    /*	// flip a coin to decide whether to send the sound forward
        ...
        if(x < threshold) {
            if(get_trace() && Trace_out)
                Trace_out << processor_info() << "sound " << psychological_name << " stream id " << stream << " is
    ...
    */
    ...
    // transmit the event forward to the next store
    schedule_event(new Auditory_Sound_Start_event(get_time() + appearance_delay.get_long_value(), sensory_store_ptr,
                                                  psychological_name, stream_name, time_stamp, location, timbre,
                                                  loudness, intrinsic_duration));
...
void Ear_processor::make_sound_stop_f(const Symbol& psychological_name)
{
    ...
    */
    schedule_event(new Auditory_Sound_Stop_event(get_time() + disappearance_delay.get_long_value(), sensory_store_ptr,
                                                 psychological_name));
}

// sound property changes are also window-delayed so that masking might happen
Symbol Ear_processor::set_sound_property(const Symbol& obj_name, const Symbol& prop_name, const Symbol& prop_value)
{
    if (get_trace() && Trace_out)
...
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set sound " << psychological_name << " property " << prop_name << " to "
                  << prop_value << endl;
    // go head and send the property forward
    long delay = property_delay.get_long_value();
    // queue a message to the sensory store
    schedule_event(new Auditory_Sound_Set_Property_event(get_time() + delay, sensory_store_ptr, psychological_name,
                                                         prop_name, prop_value));
    return prop_value;
}
...
void Ear_processor::make_new_speech_start(const Speech_word& physical_word)
{
    ...
    if (get_trace() && Trace_out)
        //		Trace_out << processor_info() << "start speech sound: " << physical_word.name << ' ' <<
        // stream_physical_name << ' '
        Trace_out << processor_info() << "schedule speech start event: " << physical_word.name << ' '
                  << physical_word.stream_name << ' ' << physical_word.content << ' ' << physical_word.speaker_gender
                  << ' ' << physical_word.speaker_id << ' ' << physical_word.pitch << ' ' << physical_word.loudness
                  << ' ' << physical_word.duration << endl;
    ...
    schedule_event(
        new Auditory_Speech_Start_event(get_time() + appearance_window.get_long_value(), this, psychological_word));
...
void Ear_processor::make_new_speech_start_f(const Speech_word& word)
{
    ...
    // word.loudness (which is used for the stream tracker) is the left level instead - contralateral level was not
    // normalized
    word_propagated.loudness = word_propagated.level_left;
    //	Normal_out << "Ear_processor: word loudness, level_right, level_left " << word_propagated.loudness << ' ' <<
    // word_propagated.level_right << ' ' << word_propagated.level_left << endl;
    ...
    schedule_event(new Auditory_Speech_Start_event(get_time() + appearance_delay.get_long_value(), sensory_store_ptr,
                                                   word_propagated));
...
double Ear_processor::apply_head_shadowing_attenuation(double original_loudness, GU::Point location)
{
    ...
    double source_level =
        original_loudness - LISTEN_band_importance_function(azimuth); // attenuate, since gain increases
    ...
    //	Normal_out << "azimuth, original, source, contra " << azimuth << ' ' << original_loudness << ' ' << source_level
    //<< ' ' << contra_level << endl;
    return contra_level;
}
...
double Ear_processor::KEMAR_2500_function(double azimuth_)
{
    ...
    double gains[37] = {-7.4431, -8.6509, -9.8394, -10.757, -11.61,  -11.365, -11.018,  -9.8432,  -8.5888, -7.5132,
                        -6.4306, -5.4092, -4.3788, -3.6004, -2.8106, -1.7451, -0.66817, -0.34874, 0.0,     0.3719,
                        0.77029, 1.3152,  1.8667,  2.175,   2.4918,  2.6518,  2.8123,   2.9494,   3.0889,  3.115,
                        3.1473,  2.9789,  2.8168,  2.4039,  1.9974,  1.6652,  1.3393};
...
double Ear_processor::LISTEN_avg_function(double azimuth_)
{
    ...
    double gains[37] = {-3.8569, -4.6028, -5.3039, -5.9152, -6.393,  -6.6978, -6.7914, -6.6506,  -6.3143,   -5.8365,
                        -5.2663, -4.633,  -3.9603, -3.2706, -2.5779, -1.8946, -1.2319, -0.59735, 0.0027228, 0.56164,
                        1.0722,  1.527,   1.9208,  2.2564,  2.539,   2.7734,  2.9641,  3.1153,   3.2305,    3.309,
                        3.3496,  3.3514,  3.3178,  3.2525,  3.1605,  3.0502,  2.9306};
    // since azimuth is integral, should result in values [0 ... 36]
    int index = int((azimuth + 90) / 5);
    Assert(index >= 0 && index < sizeof(gains));
    return gains[index];
...
double Ear_processor::LISTEN_band_importance_function(double azimuth_)
{
    ...
    double gains[37] = {-6.0042, -6.3154, -6.5939, -6.8073, -6.9263, -6.9348, -6.8203, -6.5751,  -6.2128,    -5.7521,
                        -5.2117, -4.61,   -3.9653, -3.295,  -2.6132, -1.9331, -1.2669, -0.62238, -0.0065867, 0.57396,
                        1.1142,  1.6093,  2.0552,  2.4496,  2.7909,  3.0778,  3.3117,  3.4946,   3.6282,     3.7144,
                        3.7547,  3.7515,  3.7088,  3.6313,  3.5247,  3.398,   3.2613};
    // since azimuth is integral, should result in values [0 ... 36]
    int index = int((azimuth + 90) / 5);
    Assert(index >= 0 && index < sizeof(gains));
    return gains[index];
...
double Ear_processor::get_scaled_perceived_azimuth_difference(double azimuth1, double azimuth2)
{
    ...
    /*
    // azimuth: straight ahead is 0. sounds can be either to the left, or right. azimuth difference is absolute degrees
between the two, in either
    // direction.
