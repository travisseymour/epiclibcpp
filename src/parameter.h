...
public:
    ...
    typedef enum when_randomized_e
    {
        When_used,
        On_command,
        Never
    ...
    Parameter(const std::string& name_, Distribution_type distribution_type_, Randomization_time randomization_time_,
              double mean_value_, double deviation_);
    ...
    {
        if (randomization_time == On_command)
...
private:
    // Travis: vvv this is just a type that signifies a function with 2 double arguments.
    //         fp is created to dynamically point to the approriate randomization function.
    //         I suppose the purpose of this approach is either c++ requires this approach,
    //         or DK wants to make sure no other kind of function can be assigned to fp?
