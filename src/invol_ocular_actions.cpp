...
void Invol_ocular_action::dispatch(long time)
{
    human_ptr->schedule_event(
        new Motor_Command_Action_event(time, human_ptr->get_invol_ocular_ptr(), shared_from_this()));
}
...
long Invol_ocular_Saccade_action::prepare(long base_time)
{
    if (get_processor_ptr()->get_trace() && Trace_out)
        Trace_out << get_processor_ptr()->processor_info() << "Prepare Involuntary saccade to " << get_object_name()
                  << endl;
...
long Invol_ocular_Saccade_action::execute(long base_time)
{
    ...
    Coordinator::get_instance().schedule_event(
        new Eye_Involuntary_Saccade_Start_event(base_time, get_eye_ptr(), eye_destination, movement_completion_time));
    ...
    Coordinator::get_instance().schedule_event(
        new Eye_Involuntary_Saccade_End_event(movement_completion_time + 1, get_eye_ptr()));
...
long Invol_ocular_Smooth_move_action::prepare(long base_time)
{
    compute_movement_features();
    if (get_processor_ptr()->get_trace() && Trace_out)
        Trace_out << get_processor_ptr()->processor_info() << "Prepare Involuntary smooth move to " << get_object_name()
                  << endl;
...
long Invol_ocular_Smooth_move_action::execute(long base_time)
{
    ...
    // send the eye movement start event at the current time to the eye
    Coordinator::get_instance().schedule_event(new Eye_Involuntary_Smooth_Move_Start_event(
    ...
    // the eye processor will ask for a centering to take place - DK 4/23/16
    Coordinator::get_instance().schedule_event(
        new Eye_Involuntary_Smooth_Move_End_event(movement_completion_time + 1, get_eye_ptr()));
