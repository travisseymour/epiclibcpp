...
public:
    ...
    {
        msg = proc_ptr->processor_info() + "Unknown perceptual property received: " + propname.str();
    }
};
...
void Auditory_perceptual_processor::setup()
{
    ...
    stream_tracker_ptr =
        std::move(std::unique_ptr<Stream_tracker_base>(new Stream_tracker_MinDistNoisy("Averaging_stream")));
}
...
void Auditory_perceptual_processor::set_parameter(const Parameter_specification& param_spec)
{
    if (param_spec.parameter_name == "Recoding_time") {
        ...
        if (!iss)
            Parameter::throw_parameter_error("Unable to read Detection_category name in parameter specification",
                                             param_spec);
        ...
        if (count <= 0)
            Parameter::throw_parameter_error("Did not find at least one word_content for Detection_category",
                                             param_spec);
    }
        ...
        iss >> detection_category_name;
        if (!iss)
            Parameter::throw_parameter_error("Unable to read Detection Category name in parameter specification",
                                             param_spec);
        Symbol cat_name(detection_category_name);
        ...
        if (!iss)
            Parameter::throw_parameter_error("Unable to read mean, sd, lapse rate in parameter specification",
                                             param_spec);
        ...
        iss >> category_name;
        if (!iss)
            Parameter::throw_parameter_error("Unable to read Recoding_category name in parameter specification",
                                             param_spec);
        ...
        if (count <= 0)
            Parameter::throw_parameter_error("Did not find at least one word_content for Recoding_category",
                                             param_spec);
    }
    else {
...
void Auditory_perceptual_processor::describe_parameters(Output_tee& ot) const
{
    ...
    for (const auto& pair : detection_parameters)
        ot << "    " << pair.first << ": mean = " << pair.second.mean << ", sd = " << pair.second.sd
           << ", lapse rate = " << pair.second.lapse_rate << endl;
}
...
void Auditory_perceptual_processor::make_new_sound_start(const Symbol& sound_name, const Symbol& stream_name,
                                                         long time_stamp, GU::Point location, const Symbol& timbre,
                                                         double loudness, long intrinsic_duration)
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "start sound " << sound_name << endl;

    schedule_event(new Auditory_Sound_Start_event(get_time() + appearance_window.get_long_value(), this, sound_name,
                                                  stream_name, time_stamp, location, timbre, loudness,
                                                  intrinsic_duration));
}
...
void Auditory_perceptual_processor::make_new_sound_start_f(const Symbol& sound_name, const Symbol& stream_name,
                                                           long time_stamp, GU::Point location, const Symbol& timbre,
                                                           double loudness, long intrinsic_duration)
{
    if (get_trace() && Trace_out)
    ...
    /*
        ...
        if(x < threshold) {
            if(get_trace() && Trace_out)
                Trace_out << processor_info() << "sound " << psychological_name << " stream id " << stream_name << " is
       masked" << endl; stream_id = Symbol();
            }
    */


    // transmit the start event forward to the next store
    schedule_event(new Auditory_Sound_Start_event(get_time() + appearance_delay_time, perceptual_store_ptr, sound_name,
                                                  stream_name, time_stamp, location, timbre, loudness,
                                                  intrinsic_duration));
    // create and send a transient property forward - Onset arrives at same time, then disappears
    schedule_event(new Auditory_Sound_Set_Property_event(get_time() + appearance_delay_time, perceptual_store_ptr,
                                                         sound_name, Detection_c, Onset_c));
    // arrange to remove the transient after its delay
    schedule_event(new Auditory_Sound_Set_Property_event(get_time() + appearance_delay_time +
                                                             transient_decay_time.get_long_value(),
                                                         perceptual_store_ptr, sound_name, Detection_c, Nil_c));
}
...
void Auditory_perceptual_processor::make_sound_stop_f(const Symbol& sound_name)
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "make sound stop " << sound_name << endl;
    /* since we flipped independent coins to decide whether to mask content and/or stream ID, sound was always
    propagated, so that it is stopped should be also always propagated - dk 5/25/10 */
    /*
    ...
    */
    ...
    schedule_event(new Auditory_Sound_Stop_event(get_time() + disappearance_delay.get_long_value(),
                                                 perceptual_store_ptr, sound_name));

    // create and send a transient property forward
    schedule_event(new Auditory_Sound_Set_Property_event(get_time() + disappearance_delay.get_long_value(),
                                                         perceptual_store_ptr, sound_name, Detection_c, Offset_c));
    // arrange to remove the transient after its delay
    schedule_event(new Auditory_Sound_Set_Property_event(get_time() + disappearance_delay.get_long_value() +
                                                             transient_decay_time.get_long_value(),
                                                         perceptual_store_ptr, sound_name, Detection_c, Nil_c));
    ...
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "destroy event for " << stream_name << " scheduled for " << destroy_time
                  << endl;
    schedule_event(new Auditory_Stream_Destroy_event(destroy_time, this, stream_name));
}
...
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set sound " << obj_name << " property " << prop_name << " to " << prop_value
                  << endl;

    schedule_event(new Auditory_Sound_Set_Property_event(get_time() + property_window.get_long_value(), this, obj_name,
...
// basic assumption: non-core properties are sent after a recoding time that is the intrinsic duration of the sound plus
// a recoding time not clear if this is sensible if a property changes well after the sound starts -- dk 4/19/10

Symbol Auditory_perceptual_processor::set_sound_property_f(const Symbol& sound_name, const Symbol& prop_name,
                                                           const Symbol& prop_value)
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set sound " << sound_name << " property " << prop_name << " to " << prop_value
                  << endl;
    // check to see if the sound is masked
    shared_ptr<Auditory_sound> sound_ptr = sensory_store_ptr->get_sound_ptr(sound_name);
    ...
    if (sound_ptr->get_property_value(Acoustic_status_c) == Content_masked_c) {
        if (get_trace() && Trace_out)
            Trace_out << processor_info() << "set sound " << sound_name << " property " << prop_name << " to "
                      << prop_value << " suppressed due to content masking" << endl;
        return prop_value;
    }
    ...
    long recoding_time =
        long((property_recoding_time + intrinsic_duration) * recoding_time_fluctuation.get_double_value());
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << sound_name << " property " << prop_name << " recognized after "
                  << intrinsic_duration << " + " << property_recoding_time << " for fluctuated total of "
                  << recoding_time << endl;


    // transmit the event forward to the next store
    schedule_event(new Auditory_Sound_Set_Property_event(get_time() + recoding_time, perceptual_store_ptr, sound_name,
                                                         prop_name, prop_value));
...
void Auditory_perceptual_processor::make_new_speech_start(const Speech_word& word)
{
    ...
    schedule_event(new Auditory_Speech_Start_event(get_time() + appearance_window.get_long_value(), this, word));
}
...
processed nor passed along. Instead loudness and pitch are used by the cognitive processor to associate words to
streams.
...
void Auditory_perceptual_processor::make_new_speech_start_f(const Speech_word& word)
{
    ...
    // tlschange 10/25/2021a, don't apply masking and stream tracking to ANYTHING
    //                        reason: it's not working properly
    /*
    ...
    // tlschange 10/19/2021a, don't apply masking and stream tracking to Internal Speech Events
    //                      vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
    if(n_sounds_present > 1 && word.speaker_id != Symbol("Self")) {
...
//            Normal_out << "left and right levels are different " << this_level_left << ' ' << this_level_right <<
endl;
...
//            double azimuth_difference = fabs((this_azimuth < other_azimuth) ? other_azimuth - this_azimuth :
this_azimuth - other_azimuth);
...
//		Normal_out << "best ear snr " << loudness_snr << endl;
        ...
        double azimuth_difference = Ear_processor::get_scaled_perceived_azimuth_difference(this_azimuth,
other_azimuth_mean);
        ...
        // at this point the stream assignment/updating has been done for both this sound / word and the other sound
present as well.
        ...
        Symbol this_detection_category = (detection_category_it != detection_categories.end()) ?
detection_category_it->second : Nil_c;
        ...
        //	simple detection model
        {
            ...
            content_detected = lapsed_gaussian_detection_function(snr, detection_mean, detection_sd,
detection_lapse_rate);
        }
        ...
        if(!content_detected) {
            if(get_trace() && Trace_out)
                Trace_out << processor_info() << "speech sound " << word.name << " content " << word.content << " is
masked" << endl;
        ...
        if(!stream_detected) {
            if(get_trace() && Trace_out)
                Trace_out << processor_info() << "speech sound " <<word.name << " stream id " << word.stream_name << "
is masked" << endl;
            }

    }

    */  // end tlschange 10/25/2021a
    ...
    schedule_event(new Auditory_Speech_Start_event(get_time() + appearance_delay.get_long_value(), perceptual_store_ptr,
                                                   word_propagated));

    // create and send a transient property forward - Onset arrives at same time, then disappears
    schedule_event(new Auditory_Sound_Set_Property_event(get_time() + appearance_delay.get_long_value(),
                                                         perceptual_store_ptr, word.name, Detection_c, Onset_c));
    // arrange to remove the transient after its delay
    schedule_event(new Auditory_Sound_Set_Property_event(get_time() + appearance_delay.get_long_value() +
                                                             transient_decay_time.get_long_value(),
                                                         perceptual_store_ptr, word.name, Detection_c, Nil_c));
    ...
    if (recoding_category_it != category_recodings.end() && time_it != recoding_times.end()) {
        ...
        // long recoding_time = long((property_recoding_time + duration) *
        // recoding_time_fluctuation.get_double_value());
        long recoding_time = long(
            (property_recoding_time)*recoding_time_fluctuation.get_double_value()); // dk 8/11/11 to speed things up

        schedule_event(new Auditory_Sound_Set_Property_event(get_time() + recoding_time, perceptual_store_ptr,
...
void Auditory_perceptual_processor::handle_event(const Auditory_Delay_event* event_ptr)
{
    ...
    if (!(sensory_store_ptr->is_sound_present(object_name))) {
        if (get_trace() && Trace_out)
            Trace_out << processor_info() << object_name << " no longer present for delay event of " << property_value
                      << " set to " << property_value << endl;
        return;
    }
    ...
    // check to see if the object's event time for the property and the property value matches this event; if not, do
    // nothing
    ...
    /*	if(object_time != event_ptr->get_time() || object_property_value != property_value) {
            if(get_trace() && Trace_out)
                Trace_out << processor_info() << "Delay event discarded for " << object_name
                    << " because event time and value " << event_ptr->get_time() << " " << property_value << " does not
       match stored " << object_time << " " << object_property_value << endl; return;
            }
    */
    // here if event matches - reset the stored event time, and transmit the information immediately to the perceptual
    // store and transmit the event forward to the next store
    //	obj_ptr->remove_property_event_time(property_name);

    schedule_event(new Auditory_Sound_Set_Property_event(get_time(), perceptual_store_ptr, object_name, property_name,
                                                         property_value));
}
 ...
 */
void Auditory_perceptual_processor::create_stream(const Symbol& name, double pitch_, double loudness_,
                                                  GU::Point location)
{
    if (get_trace() && Trace_out)
    ...
    // transmit the event forward to the next store
    schedule_event(new Auditory_Stream_Create_event(get_time() + appearance_delay.get_long_value(),
                                                    get_human_ptr()->get_auditory_perceptual_store_ptr(), name, pitch_,
                                                    loudness_, location));
}
...
void Auditory_perceptual_processor::destroy_stream(const Symbol& name)
{
    ...
    // transmit the event forward to the next store
    schedule_event(new Auditory_Stream_Destroy_event(get_time() + disappearance_delay.get_long_value(),
                                                     perceptual_store_ptr, name));
}
...
void Auditory_perceptual_processor::handle_event(const Auditory_Stream_Destroy_event* event_ptr)
{
    ...
    for (auto the_stream : streams) {
        if (the_stream->get_name() == stream_name && the_stream->get_disappearance_time() == event_time) {
            if (get_trace() && Trace_out)
                Trace_out << processor_info() << "stream holder" << the_stream->get_name()
                          << " deleted stream scheduled to be destroyed" << endl;
            stream_tracker_ptr->delete_stream(stream_name);
            schedule_event(new Auditory_Stream_Destroy_event(get_time() + disappearance_delay.get_long_value(),
                                                             perceptual_store_ptr, stream_name));
    ...
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "destroy event for " << stream_name << " discarded because " << event_time
                  << " not matched" << endl;
}
...
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set stream " << name << " location to " << location << endl;


    // transmit the event forward to the next store
    schedule_event(new Auditory_Stream_Set_Location_event(get_time() + location_delay.get_long_value(),
                                                          perceptual_store_ptr, name, location));
    return location;
}
...
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set stream " << name << " pitch to " << pitch << endl;


    // transmit the event forward to the next store
    schedule_event(new Auditory_Stream_Set_Pitch_event(get_time() + location_delay.get_long_value(),
                                                       perceptual_store_ptr, name, pitch));
    return pitch;
}
...
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set stream " << name << " loudness to " << loudness << endl;


    // transmit the event forward to the next store
    schedule_event(new Auditory_Stream_Set_Loudness_event(get_time() + location_delay.get_long_value(),
                                                          perceptual_store_ptr, name, loudness));
    return loudness;
}
...
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set stream " << name << " size to " << size << endl;


    // transmit the event forward to the next store
    schedule_event(new Auditory_Stream_Set_Size_event(get_time() + location_delay.get_long_value(),
                                                      perceptual_store_ptr, name, size));
    return size;
}

Symbol Auditory_perceptual_processor::set_stream_property(const Symbol& object_name, const Symbol& property_name,
                                                          const Symbol& property_value)
{
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set stream " << object_name << " property " << property_name << " to "
                  << property_value << endl;
    ...
    // transmit the event forward to the next store
    schedule_event(new Auditory_Stream_Set_Property_event(get_time() + recoding_time, perceptual_store_ptr, object_name,
