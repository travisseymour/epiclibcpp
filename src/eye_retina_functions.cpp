...
shared_ptr<Availability> Availability::create(const Visual_physical_store& physical_store,
                                              const Parameter_specification& param_spec)
{
    ...
    iss >> func_type;
    if (!iss)
        Parameter::throw_parameter_error("Unable to read availability function type in parameter specification",
                                         param_spec);
...
{
    ostringstream oss;
    oss << get_property_name() << ": Zone available inside " << zone_radius << " degrees eccentricity after "
        << transduction_delay << " ms.";
    return oss.str();
}
...
Relation_std_zone_availability::Relation_std_zone_availability(const Visual_physical_store& physical_store_,
...
{
    double eccentricity = physobj_ptr->get_eccentricity();
    return (eccentricity <= standard_fovea_radius || probability >= unit_uniform_random_variable());
}
...
{
    ostringstream oss;
    oss << get_property_name() << ": Flat available with probability " << probability << " after " << transduction_delay
        << " ms.";
    return oss.str();
}
...
shared_ptr<Availability> Selector_availability::create(const Visual_physical_store& physical_store,
                                                       const Symbol& prop_name,
                                                       const Parameter_specification& param_spec, istringstream& iss)
{
    long delay;
    iss >> delay;
...
{
    ostringstream oss;
    oss << get_property_name() << ": Custom availability if ecc < 8, .70, else .30 after " << transduction_delay
        << " ms.";
    return oss.str();
}

shared_ptr<Availability> Custom_availability::create(const Visual_physical_store& physical_store,
                                                     const Symbol& prop_name, const Parameter_specification& param_spec,
                                                     istringstream& iss)
{
    long delay;
    iss >> delay;
...
{
    ostringstream oss;
    oss << get_property_name() << ": Custom2 availability if ecc < 8, .70, else .2 after " << transduction_delay
        << " ms.";
    return oss.str();
}

shared_ptr<Availability> Custom_availability2::create(const Visual_physical_store& physical_store,
                                                      const Symbol& prop_name,
                                                      const Parameter_specification& param_spec, istringstream& iss)
{
    long delay;
...
string Linear_availability::get_description() const
{
    ...
    oss << get_property_name() << ": Linearly available with minimum size " << min_size
        << " degrees in fovea, and slope " << slope << " per degree eccentricity outside fovea after "
        << transduction_delay << " ms.";
    return oss.str();
}

shared_ptr<Availability> Linear_availability::create(const Visual_physical_store& physical_store,
                                                     const Symbol& prop_name, const Parameter_specification& param_spec,
                                                     istringstream& iss)
{
    long delay;
...
{
    ostringstream oss;
    oss << get_property_name() << ": Fixed quadratically available with intercept " << intercept << ", linear "
        << x_coeff << ", and quadratic " << x2_coeff << " components per degree eccentricity after "
        << transduction_delay << " ms.";
    return oss.str();
}

shared_ptr<Availability> Fixed_quadratic_availability::create(const Visual_physical_store& physical_store,
                                                              const Symbol& prop_name,
                                                              const Parameter_specification& param_spec,
                                                              istringstream& iss)
{
...
{
    ostringstream oss;
    oss << get_property_name() << ": Quadratically available outside eccentricity of " << zone
        << " with coefficient of variation " << coefvar << ", intercept " << intercept << ", linear " << x_coeff
        << ", and quadratic " << x2_coeff << " components per degree eccentricity and " << transduction_delay
        << " ms delay.";
    return oss.str();
}

shared_ptr<Availability> Quadratic_availability::create(const Visual_physical_store& physical_store,
                                                        const Symbol& prop_name,
                                                        const Parameter_specification& param_spec, istringstream& iss)
{
    long delay;
    ...
    iss >> coefvar;
    if (!iss)
        Parameter::throw_parameter_error("Unable to read coefficient of variation in parameter specification",
                                         param_spec);
    double intercept;
    iss >> intercept;
    if (!iss)
    ...
    return make_shared<Quadratic_availability>(physical_store, prop_name, delay, zone, coefvar, intercept, x_coeff,
                                               x2_coeff);
}
...
{
    ostringstream oss;
    oss << get_property_name() << ": P(detect) = (1 - " << lapse_prob
        << ") * P(size > x); x ~ Normal(m, s), where\nm = " << a << " + " << b << " * ecc + " << c
        << " * ecc^2,\n s = " << sd;
    return oss.str();
}
...
bool CubicConstSD::available(shared_ptr<Visual_store_object> physobj_ptr)
{
    ...
    double obj_size = (physobj_ptr->get_size().h + physobj_ptr->get_size().v) / 2.0;
    double mean =
        a + b * eccentricity + c * eccentricity * eccentricity + d * eccentricity * eccentricity * eccentricity;
    return lapsed_gaussian_detection_function(obj_size, mean, sd, lapse_prob);
}
...
shared_ptr<Availability> QuadraticCoefVarSD::create(const Visual_physical_store& physical_store,
                                                    const Symbol& prop_name, const Parameter_specification& param_spec,
                                                    istringstream& iss)
{
    long delay;
...
{
    ostringstream oss;
    oss << get_property_name() << ": Fixed exponentially available with coefficient " << coeff << ", and exponent "
        << expon << " components per degree eccentricity after " << transduction_delay << " ms.";
    return oss.str();
}

shared_ptr<Availability> Fixed_exponential_availability::create(const Visual_physical_store& physical_store,
                                                                const Symbol& prop_name,
                                                                const Parameter_specification& param_spec,
                                                                istringstream& iss)
{
...
{
    ostringstream oss;
    oss << get_property_name() << ": Exponential available with coefficient of variation " << coefvar
        << ", coefficient " << coeff << ", and exponent " << expon << " after " << transduction_delay << " ms.";
    return oss.str();
}

shared_ptr<Availability> Exponential_availability::create(const Visual_physical_store& physical_store,
                                                          const Symbol& prop_name,
                                                          const Parameter_specification& param_spec, istringstream& iss)
{
    long delay;
...
Color_quad_availability::Color_quad_availability(const Visual_physical_store& physical_store_, long delay_)
    : Fixed_quadratic_availability(physical_store_, Color_c, delay_, .5, 0.02, 0.0001)
    , red_min_size(1.5)
{
}
...
Text_availability::Text_availability(const Visual_physical_store& physical_store_, long delay_)
    ...
    Availability(physical_store_, Text_c)
...
string Text_availability::get_description() const
{
    ...
    oss << get_property_name()
        ...
        << ", linear " << x_coeff << ", and quadratic " << x2_coeff << " components per degree eccentricity after "
        << transduction_delay << " ms.";
    return oss.str();
}
 ...
 **************************************************************************************************************
 **************************************************************************************************************/
std::shared_ptr<Attention> Attention::create(const Visual_physical_store& physical_store,
                                             const Parameter_specification& param_spec)
{
    istringstream iss(param_spec.specification);
    // first term is property name
    ...
    iss >> func_type;
    if (!iss)
        Parameter::throw_parameter_error("Unable to read attention function type in parameter specification",
                                         param_spec);
...
/*
 ...
 All units are in degrees VA.
 */
long Fixed_quadratic_attention::delay(shared_ptr<Visual_store_object> physobj_ptr, double time_fluctuation,
                                      double attn_coeff)
{
    double eccentricity = physobj_ptr->get_eccentricity();
...
string Fixed_quadratic_attention::get_description() const
{
    ...
    ostringstream oss;
    oss << get_property_name() << ": Delay due to attention varies quadradatically from " << intercept
        << " at eccentricity 0."
           "\n\tThe shape of the funciton is defined by the attention coefficient and the x^2 coeff: "
        << x2_coeff;
    return oss.str();
}

std::shared_ptr<Attention> Fixed_quadratic_attention::create(const Visual_physical_store& physical_store,
                                                             const Symbol& prop_name,
                                                             const Parameter_specification& param_spec,
                                                             istringstream& iss)
{
...
string Quadratic_attention::get_description() const
{
    ...
    oss << get_property_name() << ": Delay due to attention varies quadradatically from " << intercept
        << " at eccentricity 0."
           "\n\tThe shape of the funciton is defined by the attention coefficient and the x^2 coeff: "
        << x2_coeff << " plus a random deviation: " << coefvar;
    return oss.str();
}

std::shared_ptr<Attention> Quadratic_attention::create(const Visual_physical_store& physical_store,
                                                       const Symbol& prop_name,
                                                       const Parameter_specification& param_spec, istringstream& iss)
{
    double intercept;
...
std::shared_ptr<Attention> Constant_attention_delay::create(const Visual_physical_store& physical_store,
                                                            const Symbol& prop_name,
                                                            const Parameter_specification& param_spec,
                                                            istringstream& iss)
{
