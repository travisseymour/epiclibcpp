...
public:
    ...
    {
        ostringstream oss;
        oss << proc_ptr->processor_info() << "Attempt to create duplicate auditory stream with name: " << obj_name;
        msg = oss.str();
    }
};
...
public:
    ...
    {
        ostringstream oss;
        oss << proc_ptr->processor_info() << "Attempt to access unknown auditory stream with id: " << obj_name;
        msg = oss.str();
    }
};
...
public:
    ...
    {
        ostringstream oss;
        oss << proc_ptr->processor_info() << "Attempt to create duplicate auditory sound with name: " << obj_name;
        msg = oss.str();
    }
};
...
public:
    ...
    {
        ostringstream oss;
        oss << proc_ptr->processor_info() << "Attempt to access unknown auditory sound with id: " << obj_name;
        msg = oss.str();
    }
};
...
/*** Auditory_sound ***/
// 7/2/15 TO DO: rationalize this class - it is trying to do too much

Auditory_sound::Auditory_sound(const Symbol& name_, const Symbol& stream_, long time_stamp_, GU::Point location_,
                               const Symbol& timbre_, double loudness_, long duration_, long intrinsic_duration_)
    : name(name_)
    , stream(stream_)
    ...
    , pitch(0.)
    , duration(duration_)
    , intrinsic_duration(intrinsic_duration_)
    , fading(false)
{
}

/* Auditory_sound::Auditory_sound(const Symbol& name_, const Symbol& stream_, long time_stamp_, GU::Point location_,
const Symbol& timbre_, const Symbol& content_, const Symbol& gender_, const Symbol& speaker_id_, int utterance_id_,
double loudness_, double level_right_, double level_left_, double pitch_, long duration_, long intrinsic_duration_) :
        ...
        loudness(loudness_), level_right(level_right_), level_left(level_left_), pitch(pitch_), duration(duration_),
intrinsic_duration(intrinsic_duration_), fading(false)
//		time_stamp(Coordinator::get_time())
        {}
*/
...
Auditory_sound::Auditory_sound(const Speech_word& word)
    : name(word.name)
    , stream(word.stream_name)
    , time_stamp(word.time_stamp)
    , location(word.location)
    , timbre(Speech_c)
    , content(word.content)
    , gender(word.speaker_gender)
    , speaker_id(word.speaker_id)
    , utterance_id(word.utterance_id)
    , loudness(word.loudness)
    , level_right(word.level_right)
    , level_left(word.level_left)
    , pitch(word.pitch)
    , duration(word.duration)
    , intrinsic_duration(word.duration)
    , fading(false)
//		time_stamp(Coordinator::get_time())
{
}
...
std::shared_ptr<Auditory_sound> Auditory_sound::create(const Symbol& sound_name, const Symbol& stream_name,
                                                       long time_stamp, GU::Point location, const Symbol& timbre,
                                                       double loudness, long duration, long intrinsic_duration)
{
    return std::shared_ptr<Auditory_sound>(new Auditory_sound(sound_name, stream_name, time_stamp, location, timbre,
                                                              loudness, duration, intrinsic_duration));
}
// fatter interface for speech sounds
/* Smart_Pointer<Auditory_sound> Auditory_sound::create(const Symbol& sound_name, const Symbol& stream_name, long
   time_stamp, GU::Point location, const Symbol& timbre, const Symbol& content, const Symbol& gender, const Symbol&
   speaker_id, int utterance_id, double loudness, double pitch, long duration, long intrinsic_duration) {return new
   Auditory_sound(sound_name, stream_name, time_stamp, location, timbre, content, gender, speaker_id, utterance_id,
   loudness, pitch, duration, intrinsic_duration);}
*/
...
//		{return std::shared_ptr<Auditory_sound>(new Auditory_sound(word.name, word.stream_name, word.time_stamp,
// word.location, Speech_c, word.content, word.speaker_gender, word.speaker_id, word.utterance_id, word.loudness,
// word.level_right, word.level_left, word.pitch, 0, word.duration));}
...
void Auditory_sound::display_contents(Output_tee& ot) const
{
    ...
    ot << name << ' ' << stream << ' ' << time_stamp << ' ' << location << ' ' << timbre << ' ' << content << ' '
       << gender << ' ' << speaker_id << ' ' << loudness << ' ' << duration << ' ' << intrinsic_duration << ' '
       << fading << endl;
    properties.display_contents(ot);
}
...
void Auditory_store::create_stream(const Symbol& name, double pitch, double loudness, GU::Point location)
{
    ...
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "create stream " << name << " with " << ' ' << pitch << ' ' << loudness << ' '
                  << location << endl;
...
GU::Point Auditory_store::set_stream_location(const Symbol& obj_name, GU::Point location)
{
    ...
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set stream " << obj_name << " location to " << location << endl;
...
double Auditory_store::set_stream_pitch(const Symbol& obj_name, double pitch)
{
    ...
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set stream " << obj_name << " pitch to " << pitch << endl;
    shared_ptr<Auditory_stream> obj_ptr = get_stream_ptr(obj_name); // throws if not there
...
double Auditory_store::set_stream_loudness(const Symbol& obj_name, double loudness)
{
    ...
    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set stream " << obj_name << " loudness to " << loudness << endl;
    shared_ptr<Auditory_stream> obj_ptr = get_stream_ptr(obj_name); // throws if not there
...
Symbol Auditory_store::set_stream_property(const Symbol& obj_name, const Symbol& prop_name, const Symbol& prop_value)
{
    changed = false;

    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "set stream " << obj_name << " property " << prop_name << " to " << prop_value
                  << endl;
...
// void Auditory_store::make_new_sound_start(const Symbol& sound_name, const Symbol& stream_name, long time_stamp, const
// Symbol& timbre, double loudness,
void Auditory_store::make_new_sound_start(const Symbol& sound_name, const Symbol& stream_name, long time_stamp,
                                          GU::Point location, const Symbol& timbre, double loudness,
                                          long intrinsic_duration)
{
    // create the new sound object - a continuing sound has duration 0
    //	insert_new(Auditory_sound::create(sound_name, stream_name, time_stamp, timbre, loudness, 0,
    // intrinsic_duration));
    insert_new(
        Auditory_sound::create(sound_name, stream_name, time_stamp, location, timbre, loudness, 0, intrinsic_duration));

    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "start sound " << sound_name << ' ' << stream_name << ' ' << timbre << ' '
                  << loudness << ' ' << intrinsic_duration << endl;

    notify_views(&View_base::notify_auditory_sound_start, sound_name, stream_name, time_stamp, location, timbre,
                 loudness);
...
{
    // create the new sound object - coninuing sound, so duration is 0, and intrinsic_duration is the duration of the
    // speech sound
    //	insert_new(Auditory_sound::create(name, stream, time_stamp, Speech_c, content, speaker_gender, speaker_id,
    // loudness, 0, duration));
    insert_new(Auditory_sound::create(word));

    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "start sound " << word.name << ' ' << word.stream_name << ' ' << Speech_c
                  << ' ' << word.loudness << ' ' << word.duration << ' ' << word.content << ' ' << word.speaker_gender
                  << ' ' << word.speaker_id << ' ' << endl;

    //	notify_views(&View_base::notify_auditory_speech_start, word.name, word.stream_name, word.time_stamp,
    // word.content, word.speaker_gender, word.speaker_id, word.loudness);
...
void Auditory_store::make_sound_event(const Symbol&, const Symbol&, const Symbol&, double, long, long)
{
}
...
// return the old property value
Symbol Auditory_store::set_sound_property(const Symbol& obj_name, const Symbol& prop_name, const Symbol& prop_value)
{
    changed = false;
...
{
    Auditory_store::make_new_sound_start(event_ptr->sound_name, event_ptr->stream_name, event_ptr->time_stamp,
                                         event_ptr->location, event_ptr->timbre, event_ptr->loudness,
...
{
    //	Auditory_store::make_speech_event(event_ptr->sound_name, event_ptr->stream_name, event_ptr->content,
    // event_ptr->speaker_gender, event_ptr->speaker_id, event_ptr->loudness, event_ptr->duration);
