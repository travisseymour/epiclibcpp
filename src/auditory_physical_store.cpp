                                               ...
                                               long intrinsic_duration)
{
    ...
    insert_new(Auditory_sound::create(physical_name, physical_stream_name, time_stamp, location, timbre, loudness, 0,
                                      intrinsic_duration));

    if (get_trace() && Trace_out)
        Trace_out << processor_info() << "start sound " << physical_name << ' ' << physical_stream_name << ' ' << timbre
                  << ' ' << intrinsic_duration << endl;

    notify_views(&View_base::notify_auditory_sound_start, physical_name, physical_stream_name, time_stamp, location,
                 timbre, loudness);

    changed = true;
    // inform the next processor in terms of psychological name only
    get_human_ptr()->get_Ear_processor_ptr()->make_new_sound_start(physical_name, physical_stream_name, time_stamp,
                                                                   location, timbre, loudness, intrinsic_duration);
}
...
{
    // create the new sound object - a continuing sound has duration 0; use duration of speech unit as the
    // intrinsic_duration odf the sound
    long time_stamp = Coordinator::get_time();
    ...
    /*	notify_views(&View_base::notify_auditory_speech_start, physical_word.name, stream_physical_name, time_stamp,
       physical_word.content, physical_word.speaker_gender, physical_word.speaker_id, physical_word.loudness,
       physical_word.pitch); */
...
void Auditory_physical_store::make_sound_event(const Symbol& physical_name, const Symbol& stream_name,
                                               GU::Point location, const Symbol& timbre, double loudness, long duration,
                                               long intrinsic_duration)
{
    // start the sound
...
void Auditory_physical_store::handle_event(const Auditory_Speech_event* event_ptr)
{
    ...
    */
    //	make_speech_event(event_ptr->sound_name, event_ptr->stream_name, event_ptr->content,event_ptr->speaker_gender,
    // event_ptr->speaker_id, event_ptr->loudness, event_ptr->duration);
...
Symbol Auditory_physical_store::set_sound_property(const Symbol& physical_name, const Symbol& propname,
