...
GU::Point Widget::screen_pixel_visual_origin = GU::Point(screen_hor_pixel_size / 2., screen_vert_pixel_size / 2.);
...
void Widget::present_object()
{
    ...
    device_ptr->make_visual_object_appear(widget_name, pixel_point_to_dva_point(center_location),
                                          pixel_size_to_dva_size(size));
    presented = PRESENTED;
}
...
bool Widget::is_inside_container() const
{
    ...
    return (top_edge >= c_top_edge && bottom_edge <= c_bottom_edge && left_edge >= c_left_edge &&
...
Polygon_widget::Polygon_widget(Device_base* device_ptr_, const Symbol& widget_name_, const GU::Polygon& polygon,
                               const Symbol& color_)
...
Line_widget::Line_widget(Device_base* device_ptr_, const Symbol& widget_name_, const GU::Line_segment& line,
                         const Symbol& color_)
...
void Cursor_widget::present_object()
{
    ...
    get_device_ptr()->make_visual_object_appear(get_name(), pixel_point_to_dva_point(center_location),
                                                pixel_size_to_dva_size(get_size()));
...
Labeled_field_widget::Labeled_field_widget(Device_base* device_ptr, const Symbol& widget_name, GU::Point location,
                                           GU::Size label_size, GU::Size field_size, const Symbol& label,
                                           const Symbol& contents, const Symbol& color)
    : Widget(device_ptr, widget_name, location,
             GU::Size(label_size.h + field_size.h, ((label_size.v > field_size.v) ? label_size.v : field_size.v)))
{
    ...
    field_ptr.reset(new Field_widget(device_ptr, value_name, GU::Point(location.x + label_size.h, location.y),
                                     field_size, contents, color));
}
