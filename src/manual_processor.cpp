...
/* Aimed Actions
Aimed actions all imply that some visually-locable object (the mouse cursor, the finger) is going to be moved to the
position of some other located object.
*/
/*
void Manual_processor::handle_event(const Manual_Point_Mouse_event * event_ptr)
{
    ...
    start_command(new Manual_Point_Mouse_action(this, event_ptr->cursor, event_ptr->target,
event_ptr->execute_when_prepared));
}
*/
...
// if time is a multiple of 50 ms, subtract one to align with next cognitive cycle.
    ...
    Message * event_ptr = new Cognitive_Manual_Operation_Complete_Message (signal_time, cognitive_ptr,
facade_ptr->get_broadcaster());
