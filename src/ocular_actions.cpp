...
public:
    ...
    {
        ostringstream oss;
        oss << "*** Error: Attempt to move eyes to object " << obj_name << " at location: " << location
            << " outside of " << bounding_box;
        msg = oss.str();
    }
};
...
shared_ptr<Motor_action> Ocular_action::create(Human_processor* human_ptr, Symbol_list_t arguments,
...
{
    human_ptr->schedule_event(new Motor_Command_Action_event(time, human_ptr->get_ocular_ptr(), shared_from_this()));
}
...
void Ocular_Saccade_action::set_target_object_source_and_location(GU::Point opt_loc)
{
    ...
    bool found;
    obj_location =
        human_ptr->get_Cognitive_processor_ptr()->get_named_location(obj_name, found); // awkward - no is_present?
    if (found) {
...
long Ocular_Saccade_action::prepare(long base_time)
{
    ...
    //	if(move_vector.r > 2.5) { // try without 12/4/14
    {
        ...
        if (SD > 0.0) {
            do {
                actual_distance = normal_random_variable(base_distance, SD);
...
long Ocular_Saccade_action::execute(long base_time)
{
    ...
    Coordinator::get_instance().schedule_event(
        new Eye_Voluntary_Saccade_Start_event(base_time, get_eye_ptr(), eye_destination, movement_completion_time));
    ...
    Coordinator::get_instance().schedule_event(
        new Eye_Voluntary_Saccade_End_event(movement_completion_time + 1, get_eye_ptr(), true, false));
    ...
    */
    ...
    Coordinator::get_instance().schedule_event(
        new Device_Eyemovement_Start_event(base_time, human_ptr->get_device_ptr(), obj_device_name, eye_destination));

    /*	Coordinator::get_instance().schedule_event(new
            Device_Eyemovement_End_event(
                movement_completion_time, human_ptr->get_device_ptr(), obj_device_name,
       get_target_object_location()));*/
    // see above - DK 4/23/16
    Coordinator::get_instance().schedule_event(new Device_Eyemovement_End_event(
...
Ocular_Random_Move_action::Ocular_Random_Move_action(Human_processor* human_ptr_, const Symbol_list_t& arguments,
                                                     bool execute_when_prepared)
...
Ocular_Mode_action::Ocular_Mode_action(Human_processor* human_ptr_, const Symbol_list_t& arguments,
                                       bool execute_when_prepared)
    : Ocular_action(human_ptr_, execute_when_prepared)
{
...
long Ocular_Mode_action::execute(long base_time)
{
    ...
    if (get_processor_ptr()->get_trace() && Trace_out)
        Trace_out << get_processor_ptr()->processor_info() << "Mode Command: " << command << ' ' << system << endl;
...
Ocular_Look_for_action::Ocular_Look_for_action(Human_processor* human_ptr, const Symbol_list_t& arguments,
                                               bool execute_when_prepared)
    : Ocular_action(human_ptr, execute_when_prepared)
    , pv_list(arguments)
{
...
long Ocular_Look_for_action::execute(long base_time)
{
    ...
    // send the eye movement start event at the current time to the eye
    Coordinator::get_instance().schedule_event(
        new Eye_Voluntary_Saccade_Start_event(base_time, get_eye_ptr(), obj_location, movement_completion_time));

    // send the eye movement end event at the movement_completion_time time to the eye
    // set centering on, reflexes off
    Coordinator::get_instance().schedule_event(
        new Eye_Voluntary_Saccade_End_event(movement_completion_time, get_eye_ptr(), true, false));
