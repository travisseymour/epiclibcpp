...
public:
    Negation_Node(Combining_Node* in_pos, Combining_Node* in_neg)
        : input_positive(in_pos)
        , input_negative(in_neg)
    {
    }
...
public:
    ...
    virtual void display_contents(const char* label, bool dynamic, int indent_level) const
    {
        ...
        PPS_out << "Inputs: Bindng +:" << input_positive->get_id() << ", Bindng -:" << input_negative->get_id()
                << std::endl;
        indent_level++;
...
protected:
    ...
    virtual bool positive_update_from_positive()
    {
        ...
        bool changed =
            binding_sets.add_mod_if_not_match_source(pos_pos_match_type, input_positive->get_delta_binding_sets(),
                                                     input_negative->get_binding_sets(), delta_binding_sets);
    ...
    virtual bool positive_update_from_negative()
    {
        ...
        bool changed = binding_sets.remove_if_match(pos_neg_match_type, input_negative->get_delta_binding_sets(),
                                                    delta_binding_sets);
        // if binding sets didn't change, nothing further to do.
        if (!changed)
            return false;
    ...
    virtual bool negative_update_from_positive()
    {
        ...
        // output delta will be the removed sets, propagate as a removal
        bool changed = binding_sets.remove_if_match(neg_pos_match_type, input_positive->get_delta_binding_sets(),
                                                    delta_binding_sets);
        // if binding sets didn't change, nothing further to do.
        if (!changed)
            return false;
    ...
    virtual bool negative_update_from_negative()
    {
        ...
        // negative node is source of decrement; add binding sets from positive that are consistent with the negative
        // delta output delta will be binding sets added
        bool changed =
            binding_sets.add_mod_if_match_source(neg_neg_match_type, input_positive->get_binding_sets(),
...
class Full_Overlap_Negation_Node : public Bindings_Negation_Node<Full_Overlap, Full_Overlap, Full_Overlap, Full_Overlap>
{
public:
    Full_Overlap_Negation_Node(int in_id, Combining_Node* in_pos, Combining_Node* in_neg)
        : Bindings_Negation_Node<Full_Overlap, Full_Overlap, Full_Overlap, Full_Overlap>::Bindings_Negation_Node(
              in_id, in_pos, in_neg)
    {
    }
...
public:
    Partial_Overlap_Negation_Node(int in_id, Combining_Node* in_pos, Combining_Node* in_neg)
        : Bindings_Negation_Node<Partial_Overlap, Partial_Overlap, Full_Overlap,
                                 Partial_Overlap>::Bindings_Negation_Node(in_id, in_pos, in_neg)
    {
    }
