...
{
public:
    static std::shared_ptr<Auditory_stream> create(const Symbol& name, double pitch, double loudness,
                                                   GU::Point location)
    {
...
protected:
    ...
    //	Auditory_sound(const Symbol& name, const Symbol& stream, long time_stamp, GU::Point location, const Symbol&
    // timbre, 			const Symbol& content, const Symbol& gender, const Symbol& speaker_id, int utterance_id_, double
    // pitch, double loudness, double level_right_, double level_left, long duration, long intrinsic_duration);
...
public:
    ...
    */
    ...
    virtual void make_sound_event(const Symbol& name, const Symbol& stream, const Symbol& timbre, double loudness,
                                  long duration, long intrinsic_duration = 0);
    //	virtual void make_speech_event(const Symbol& sound_name, const Symbol& stream_name,
    //		const Symbol& content, const Symbol& speaker_gender, const Symbol& speaker_id, double loudness, long
    // duration);
    virtual void make_speech_event(const Speech_word&);

    // psychological interface -- note that sound_name, stream_name, and time_stamp come first for these functions,
    // followed by sound-specific parameters
    ...
    */
    ...
    // see if the object is present in the store and is not fading; return true if so, false if not - no exception
    // thrown
